...
async def validate_maci_record_pqc(
    ...
    if isinstance(agent_id, str) and isinstance(target_output_id, str):
...
def _extract_message_content(data: dict[str, object]) -> bytes:
    """
...
def _verify_classical_component(
    ...
    try:
        ...
        classical_component = hybrid_sig.classical
        sig_bytes = (
            classical_component.signature
            if classical_component is not None and hasattr(classical_component, "signature")
            else b""
        )
        public_key_bytes = keys.get("classical")
...
def _verify_pqc_component(
    ...
    try:
        ...
        pqc_component = hybrid_sig.pqc
        if pqc_component is None:
            return False
        pqc_alg = normalize_algorithm_name(pqc_component.algorithm)
        sig_bytes = pqc_component.signature if hasattr(pqc_component, "signature") else b""
        public_key_bytes = keys.get("pqc")
        if not isinstance(public_key_bytes, bytes):
            return False
        return bool(
            pqc_verify_signature(
...
async def _check_key_status_for_validation(
    data: dict[str, object],
    signature_data: dict[str, object],
    errors: list[str],
    warnings: list[str],
    expected_hash: str,
    start_time: float,
...
async def _verify_hybrid_signature(
    ...
    try:
        ...
        return ValidationResult(
            ...
            pqc_metadata=PQCMetadata(
                pqc_enabled=True,
                pqc_algorithm=hybrid_sig.pqc.algorithm if hybrid_sig.pqc is not None else None,
...
async def _extract_public_keys(
    data: dict[str, object], signature_data: dict[str, object]
) -> dict[str, object]:
    """
