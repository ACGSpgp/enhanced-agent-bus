...
if TYPE_CHECKING:
    from .invariant_guard import ProposalInvariantValidator as _ProposalInvariantValidator
else:
    _ProposalInvariantValidator = object

# Import invariant validation — fail-closed: log ERROR if unavailable
_INVARIANT_IMPORTS_AVAILABLE = True
_RuntimeProposalInvariantValidator: Any | None
_RuntimeConstitutionalInvariantViolation: Any | None
_get_default_manifest: Any | None
try:
    from .invariant_guard import (
        ConstitutionalInvariantViolation as _RuntimeConstitutionalInvariantViolation,
    )
    from .invariant_guard import ProposalInvariantValidator as _RuntimeProposalInvariantValidator
    from .invariants import get_default_manifest as _get_default_manifest
except ImportError:
    _INVARIANT_IMPORTS_AVAILABLE = False
    _RuntimeProposalInvariantValidator = None
    _RuntimeConstitutionalInvariantViolation = None
    _get_default_manifest = None

ProposalInvariantValidator = cast(Any, _RuntimeProposalInvariantValidator)
ConstitutionalInvariantViolation = cast(Any, _RuntimeConstitutionalInvariantViolation)
get_default_manifest = cast(Any, _get_default_manifest)
...
try:
    ...
    class MACIEnforcer:  # type: ignore[no-redef]
        ...
        async def validate_action(self, *args: object, **kwargs: object) -> object:
            raise _raise_missing_maci_dependency()
...
try:
    ...
    class ImpactScorer:  # type: ignore[no-redef]
        ...
        async def score_message(self, *args: object, **kwargs: object) -> ImpactAnalysis:
...
if TYPE_CHECKING:
    ...
    _ProposalBaseError = ACGSBaseError
...
class ProposalValidationError(_ProposalBaseError):
...
class ProposalRequest(_PydanticBaseModel):
...
class ProposalResponse(_PydanticBaseModel):
...
class AmendmentProposalEngine:
    ...
    def __init__(
        ...
        self.impact_scorer = impact_scorer or ImpactScorer(
            model_path=_os.environ.get("IMPACT_SCORER_MODEL_DIR") or None,
        )
    ...
    def _get_invariant_validator(self) -> Any | None:
        """Lazy-initialize the invariant validator."""
        if self._invariant_validator is None and ProposalInvariantValidator is not None:
            try:
                manifest = cast(Any, get_default_manifest)()
                validator_factory = cast(Any, ProposalInvariantValidator)
                self._invariant_validator = validator_factory(manifest)
            except _PROPOSAL_ENGINE_OPERATION_ERRORS as e:
                logger.warning(
                    "proposal_invariant_validator_init_failed",
                    constitutional_hash=CONSTITUTIONAL_HASH,
                    error=str(e),
                )
    ...
    async def create_proposal(self, request: ProposalRequest) -> ProposalResponse:
        ...
        if self.enable_maci and self.maci_enforcer:
            ...
            allowed = False
            if isinstance(maci_result, dict):
                allowed = bool(maci_result.get("allowed", False))
            elif hasattr(maci_result, "is_valid"):
                allowed = bool(cast(Any, maci_result).is_valid)
            if not allowed:
                error_msg = (
    ...
    async def list_proposals(
        ...
        return cast(list[AmendmentProposal], proposals)
    ...
    async def _log_audit_event(self, event_type: str, agent_id: str, details: JSONDict) -> None:
        ...
        try:
            audit_log_event = getattr(self.audit_client, "log_event", None)
            if not callable(audit_log_event):
                return
