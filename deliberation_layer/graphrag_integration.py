...
if TYPE_CHECKING:
    from src.core.cognitive.graphrag.factory import VectorSearchResult
...
class GraphRAGContextEnricher:
    ...
    def __init__(
        ...
        self._seeded_metadata: dict[str, dict[str, object]] = {}
    ...
    async def _get_vector_store(self) -> VectorStore:
        if self._vector_store is None:
            ...
            self._vector_store = cast(Any, create_vector_store("memory"))
    ...
    async def _retrieve(self, query: str, tenant_id: str) -> JSONDict:
        ...
        raw_results = cast(list[Any], await store.search(query_vector, top_k=self._top_k * 2))
        ...
        try:
            ...
            for r in raw_results:
                ...
                node = GraphNode(
                    ...
                    embedding=cast(list[float], embedding) if embedding is not None else None,
                    metadata=meta,
                )
                nodes.append(node)
