...
class ImmutableStateReducer(BaseStateReducer):
    ...
    def reduce(
        ...
        return GraphState(
            data=cast(JSONDict, safe_copy(node_output)),
    ...
    def reduce(
        ...
        if self.deep_merge:
            new_data = self._deep_merge(
                cast(JSONDict, safe_copy(current_state.data)),
                node_output,
            )
        else:
    ...
    def _deep_merge(self, base: JSONDict, updates: JSONDict) -> JSONDict:
        ...
        for key, value in updates.items():
            if key in result and isinstance(result[key], dict) and isinstance(value, dict):
                result[key] = self._deep_merge(cast(JSONDict, result[key]), cast(JSONDict, value))
    ...
    **kwargs: object,
    ...
    if strategy == "merge":
        return MergeStateReducer(
            deep_merge=bool(kwargs.get("deep_merge", False)),
            merge_lists=bool(kwargs.get("merge_lists", False)),
            constitutional_hash=constitutional_hash,
        )
    elif strategy == "overwrite":
        return OverwriteStateReducer(
            overwrite_keys=cast(list[str] | None, kwargs.get("overwrite_keys")),
            preserve_keys=cast(list[str] | None, kwargs.get("preserve_keys")),
            remove_keys=cast(list[str] | None, kwargs.get("remove_keys")),
            constitutional_hash=constitutional_hash,
        )
        ...
        return AccumulatorStateReducer(
            accumulate_keys=cast(list[str] | None, kwargs.get("accumulate_keys")),
            max_accumulate_size=int(str(kwargs.get("max_accumulate_size", 1000))),
            constitutional_hash=constitutional_hash,
        )
        ...
        return CustomStateReducer(
            reduce_fn=cast(Callable[[JSONDict, JSONDict, str], JSONDict], reduce_fn),
            constitutional_hash=constitutional_hash,
        )
    else:
