...
'''
    ...
    def _run_container_sync(self, config: dict[str, object]) -> SandboxExecutionResult:
        ...
        """
        container = None
        container_id = str(config.get("name", ""))
        ...
        try:
            ...
            volumes = cast(dict[str, object], config.get("volumes", {}))
            if volumes:
                temp_dir = str(list(volumes.keys())[0])

            if temp_dir:
                output_file = Path(temp_dir) / "output.json"
                if output_file.exists():
                    try:
                        output = cast(JSONDict, json.loads(output_file.read_text()))
            ...
            if container:
                try:
                    ...
                    self._containers.discard(str(container_id))
                except (
                    AttributeError,
                    OSError,
                    RuntimeError,
    ...
    async def health_check(self) -> dict[str, object]:
        ...
        try:
            ...
            version_info = await loop.run_in_executor(None, self._client.version)
...
async def get_default_provider() -> SandboxProvider:
    ...
    mock_provider: SandboxProvider = MockSandboxProvider()
