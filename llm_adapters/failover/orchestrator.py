...
class LLMFailoverOrchestrator:
    ...
    async def execute_with_failover(
        ...
        if critical and hedge_count > 1:
            ...
            hedged_result: tuple[str, object] = await self.hedging_manager.execute_hedged(
        ...
        try:
            provider_result: object = await execute_fn(provider)
            ...
            return provider, provider_result
            ...
            for fallback in fallbacks:
                try:
                    fb_start = time.time()
                    fallback_result: object = await execute_fn(fallback)
