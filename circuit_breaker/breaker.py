...
from .metrics import (
    acgs_circuit_breaker_failures_total as _raw_failures_total,
)
from .metrics import (
    acgs_circuit_breaker_queue_size as _raw_queue_size,
)
from .metrics import (
    acgs_circuit_breaker_rejections_total as _raw_rejections_total,
)
from .metrics import (
    acgs_circuit_breaker_state as _raw_state_metric,
)
from .metrics import (
    acgs_circuit_breaker_state_changes_total as _raw_state_changes_total,
)
from .metrics import (
    acgs_circuit_breaker_successes_total as _raw_successes_total,
)


class _CounterLike(Protocol):
    def inc(self, value: float = 1.0, **kwargs: object) -> None: ...


class _GaugeLike(_CounterLike, Protocol):
    def set(self, value: float = 0.0, **kwargs: object) -> None: ...


class _CounterMetricLike(Protocol):
    def labels(self, **kwargs: object) -> _CounterLike: ...


class _GaugeMetricLike(Protocol):
    def labels(self, **kwargs: object) -> _GaugeLike: ...


acgs_circuit_breaker_state = cast(_GaugeMetricLike, _raw_state_metric)
acgs_circuit_breaker_state_changes_total = cast(_CounterMetricLike, _raw_state_changes_total)
acgs_circuit_breaker_failures_total = cast(_CounterMetricLike, _raw_failures_total)
acgs_circuit_breaker_successes_total = cast(_CounterMetricLike, _raw_successes_total)
acgs_circuit_breaker_rejections_total = cast(_CounterMetricLike, _raw_rejections_total)
acgs_circuit_breaker_queue_size = cast(_GaugeMetricLike, _raw_queue_size)
...
class ServiceCircuitBreaker:
    ...
    def is_closed(self) -> bool:
        ...
        return bool(self._state == CircuitState.CLOSED)

    @property
    def is_open(self) -> bool:
        """Check if circuit is open (failing fast)."""
        return bool(self._state == CircuitState.OPEN)

    @property
    def is_half_open(self) -> bool:
        """Check if circuit is half-open (testing recovery)."""
        return bool(self._state == CircuitState.HALF_OPEN)
    ...
    async def _check_timeout_expiry(self) -> bool:
        ...
        return bool(elapsed >= self.config.timeout_seconds)
    ...
    async def can_execute(self) -> bool:
        ...
        async with self._lock:
            ...
            if self._state == CircuitState.HALF_OPEN:
                return bool(self._half_open_successes < self.config.half_open_requests)
    ...
    def get_cached_fallback(self, key: str) -> object | None:
        ...
        return cast(object | None, self._fallback_cache[key])
