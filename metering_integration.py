...
from typing import TYPE_CHECKING, Any, Protocol, TypeAlias, TypedDict, TypeVar, cast
...
try:
    ...
    try:
        ...
        from app.models import (
            CONSTITUTIONAL_HASH as _APP_CONSTITUTIONAL_HASH,
        )
        from app.models import (
            MeterableOperation as _APP_MeterableOperation,
        )
        from app.models import (
            MeteringTier as _APP_MeteringTier,
        )
        from app.models import (
            UsageEvent as _APP_UsageEvent,
        )
        from app.service import UsageMeteringService as _APP_UsageMeteringService

        CONSTITUTIONAL_HASH = _APP_CONSTITUTIONAL_HASH
        MeterableOperation = _APP_MeterableOperation
        MeteringTier = _APP_MeteringTier
        UsageEvent = _APP_UsageEvent
        UsageMeteringService = _APP_UsageMeteringService
...
class _UsageMeteringServiceProtocol(Protocol):
...
MeterableOperationT: TypeAlias = object
MeteringTierT: TypeAlias = object


class _MeteringEvent(TypedDict):
    tenant_id: str
    operation: MeterableOperationT | None
    tier: MeteringTierT
    agent_id: str | None
...
def _get_metering_service_factory() -> Callable[..., _UsageMeteringServiceProtocol] | None:
    if not METERING_AVAILABLE or UsageMeteringService is None:
        return None
    return cast(Callable[..., _UsageMeteringServiceProtocol], UsageMeteringService)


def _get_metering_operation(name: str) -> MeterableOperationT:
    return cast(MeterableOperationT, getattr(cast(Any, MeterableOperation), name))


def _get_metering_tier(name: str) -> MeteringTierT:
    return cast(MeteringTierT, getattr(cast(Any, MeteringTier), name))
...
class AsyncMeteringQueue:
    ...
    def __init__(
        ...
        metering_service: _UsageMeteringServiceProtocol | None = None,
    ):
        self.config = config
        self._queue: asyncio.Queue[_MeteringEvent] = asyncio.Queue(maxsize=config.max_queue_size)
    ...
    async def start(self) -> None:
        ...
        if self._metering_service is None and METERING_AVAILABLE:
            service_factory = _get_metering_service_factory()
            if service_factory is not None:
                self._metering_service = service_factory(
    ...
    def enqueue_nowait(
        ...
        if tier is None:
            tier = _get_metering_tier("STANDARD")

        event_data: _MeteringEvent = {
            "tenant_id": tenant_id,
            "operation": operation,
            "tier": tier,
            "agent_id": agent_id,
...
class MeteringHooks:
    ...
    def on_constitutional_validation(
        ...
        self._queue.enqueue_nowait(
            ...
            operation=_get_metering_operation("CONSTITUTIONAL_VALIDATION")
    ...
    def on_agent_message(
        ...
        self._queue.enqueue_nowait(
            tenant_id=tenant_id or "default",
            operation=_get_metering_operation("AGENT_MESSAGE") if METERING_AVAILABLE else None,
            tier=tier,
            agent_id=from_agent,
            latency_ms=latency_ms,
            compliance_score=1.0 if is_valid else 0.0,
    ...
    def on_policy_evaluation(
        ...
        """
        if tier is None and METERING_AVAILABLE:
            tier = _get_metering_tier("ENHANCED")
    ...
    def on_deliberation_request(
        ...
        self._queue.enqueue_nowait(
            tenant_id=tenant_id or "default",
            operation=_get_metering_operation("DELIBERATION_REQUEST"),
            tier=_get_metering_tier("DELIBERATION"),
            agent_id=agent_id,
            latency_ms=latency_ms,
    ...
    def on_hitl_approval(
        ...
        self._queue.enqueue_nowait(
            tenant_id=tenant_id or "default",
            operation=_get_metering_operation("HITL_APPROVAL"),
            tier=_get_metering_tier("DELIBERATION"),
            agent_id=agent_id,
            latency_ms=latency_ms,
...
def _get_first_non_self_arg(args: tuple[object, ...]) -> object | None:
...
def _extract_tenant_id(arg: object, extract_tenant: Callable[..., str] | None) -> str:
    ...
    if hasattr(arg, "tenant_id"):
        return arg.tenant_id or "default"
...
def _record_metering_event(
    ...
    hooks._queue.enqueue_nowait(
        tenant_id=tenant_id,
        operation=operation,
        tier=tier or _get_metering_tier("STANDARD"),
        agent_id=agent_id,
        latency_ms=latency_ms,
        compliance_score=1.0 if is_valid else 0.0,
        metadata={
