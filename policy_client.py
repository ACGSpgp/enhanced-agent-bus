...
try:
    ...
    from validators import ValidationResult as _StandaloneValidationResult

    _RuntimeValidationResult = _StandaloneValidationResult
else:
    _RuntimeValidationResult = _PackageValidationResult
...
class _CachedPolicyEntry(TypedDict):
    content: JSONDict
    timestamp: float
...
class PolicyRegistryClient:
    ...
    def __init__(
        ...
        self._cache: OrderedDict[str, _CachedPolicyEntry] = OrderedDict()
    ...
    async def _get_http_client(self) -> httpx.AsyncClient:
        """Return an initialized HTTP client."""
        if self._http_client is None:
            await self.initialize()
        client = self._http_client
        if client is None:
            raise RuntimeError("Policy registry HTTP client failed to initialize")
        return client
    ...
    async def get_policy_content(
        ...
        try:
            ...
            content = self._json_object(response)
    ...
    async def validate_message_signature(self, message: _MessageLike) -> "ValidationResult":
        """
        ...
        try:
            ...
            return _RuntimeValidationResult(
    ...
    def _validate_message_content(
        self, message: _MessageLike, policy_content: JSONDict
    ...
    def _check_message_length(self, message: _MessageLike, policy_content: JSONDict) -> list[str]:
    ...
    def _check_allowed_topics(self, message: _MessageLike, policy_content: JSONDict) -> list[str]:
    ...
    async def get_current_public_key(self) -> str | None:
        ...
        try:
            client = await self._get_http_client()
            response = await client.get(f"{self.registry_url}/api/v1/public-keys")
            response.raise_for_status()
            data = self._json_object(response)
            public_key = data.get("current_public_key")
    ...
    async def health_check(self) -> JSONDict:
        ...
        try:
            client = await self._get_http_client()
            response = await client.get(f"{self.registry_url}/health/ready")
            response.raise_for_status()
