...
class TensorRTOptimizer:
    ...
    def _load_torch_model(self) -> Any:
        ...
        if self._torch_model is None:
            ...
            cast(Any, self._torch_model).eval()

            # Move to GPU if available
            if cast(Any, torch).cuda.is_available():
                self._torch_model = cast(Any, self._torch_model).cuda()
    ...
    def convert_to_tensorrt(self, force: bool = False) -> Path | None:
        ...
        config.set_memory_pool_limit(trt_module.MemoryPoolType.WORKSPACE, 1 << 30)  # 1GB
    ...
    def load_tensorrt_engine(self) -> bool:
        ...
        try:
            ...
            self._trt_context = cast(Any, self._trt_engine).create_execution_context()
    ...
    def load_onnx_runtime(self) -> bool:
        ...
        providers = []
        if "CUDAExecutionProvider" in ort_module.get_available_providers():
            providers.append("CUDAExecutionProvider")
            logger.info("Using CUDA execution provider")
        providers.append("CPUExecutionProvider")

        self._onnx_session = ort_module.InferenceSession(
            str(self.onnx_path),
            providers=providers,
        )
    ...
    def infer_batch(self, texts: list[str]) -> Any:
        """
    ...
    def _generate_fallback_embeddings(self, batch_size: int) -> Any:
    ...
    def _infer_tensorrt(self, inputs: dict[str, Any]) -> Any:
    ...
    def _infer_onnx(self, inputs: dict[str, Any]) -> Any:
        ...
        if RUST_AVAILABLE:
            try:
                ...
                return cast(Any, rust_ops).mean_pooling_f32(last_hidden_state, attention_mask)
            except (AttributeError, OSError, RuntimeError, TypeError, ValueError) as e:
    ...
    def _infer_torch(self, inputs: dict[str, Any]) -> Any:
        """Fallback PyTorch inference."""
        if not NUMPY_AVAILABLE:
            raise ImportError("numpy is required for PyTorch inference")
        model = self._load_torch_model()

        input_ids = cast(Any, torch).from_numpy(inputs["input_ids"])
        attention_mask = cast(Any, torch).from_numpy(inputs["attention_mask"])
        ...
        if RUST_AVAILABLE:
            try:
                ...
                return cast(
                    Any,
                    cast(Any, rust_ops).mean_pooling_f32(
        ...
        sum_embeddings = cast(Any, torch).sum(last_hidden_state * input_mask_expanded, dim=1)
        sum_mask = cast(Any, torch).clamp(input_mask_expanded.sum(dim=1), min=1e-9)

        embeddings = sum_embeddings / sum_mask

        return cast(Any, embeddings.cpu().numpy())
