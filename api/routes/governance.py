...
def _load_governance_dependency() -> GovernanceFactory:
    ...
    governance_factory: GovernanceFactory | None = None
    try:
        from ...governance.ccai_framework import get_ccai_governance as _governance_factory_pkg
    except ImportError:
        pass
    else:
        governance_factory = cast(GovernanceFactory, _governance_factory_pkg)
        return governance_factory

    try:
        from governance.ccai_framework import get_ccai_governance as _governance_factory_root
    except ImportError:
        pass
    else:
        governance_factory = cast(GovernanceFactory, _governance_factory_root)
        return governance_factory
...
async def get_stability_metrics(
    ...
    return StabilityMetricsResponse(**cast(dict[str, Any], stats))
...
def _load_enforcement_mode_config_service_type() -> type[Any] | None:
    ...
    for module_name, package in (
        ("...pqc_enforcement_config", __package__),
        ("pqc_enforcement_config", None),
    ):
        try:
            module = import_module(module_name, package=package)
        except ImportError:
            continue
        service_type = getattr(module, "EnforcementModeConfigService", None)
        if isinstance(service_type, type):
            return service_type
    return None


def _load_pqc_validators() -> tuple[
    Callable[..., Any] | None,
    Callable[..., Any] | None,
]:
    """Load PQC enforcement validation helpers with shared/standalone fallbacks."""
    for module_name, package in (
        ("...pqc_validators", __package__),
        ("pqc_validators", None),
    ):
        try:
            module = import_module(module_name, package=package)
        ...
        create_fn = getattr(module, "check_enforcement_for_create", None)
        update_fn = getattr(module, "check_enforcement_for_update", None)
        if callable(create_fn) and callable(update_fn):
            return cast(Callable[..., Any], create_fn), cast(Callable[..., Any], update_fn)
    return None, None
...
def _load_pqc_error(name: str) -> type[Exception] | None:
    ...
    for module_name, package in (
        ("enhanced_agent_bus._compat.security.pqc", None),
        ("...pqc_enforcement_models", __package__),
        ("pqc_enforcement_models", None),
    ):
        try:
            module = import_module(module_name, package=package)
        except ImportError:
...
ClassicalKeyRejectedError = _load_pqc_error("ClassicalKeyRejectedError")
MigrationRequiredError = _load_pqc_error("MigrationRequiredError")
PQCKeyRequiredError = _load_pqc_error("PQCKeyRequiredError")
UnsupportedPQCAlgorithmError = _load_pqc_error("UnsupportedPQCAlgorithmError")
PQCRejectionError = _load_pqc_error("PQCRejectionError")

_PQC_ENFORCEMENT_ERRORS: tuple[type[Exception], ...] = tuple(
    e
    for e in (
...
async def register_maci_agent(
    ...
    registry: Annotated[MACIRoleRegistry | None, Depends(_get_maci_registry)] = None,
) -> MACIAgentResponse:
...
async def validate_maci_action(
    ...
    _tenant_id: str = Depends(get_tenant_id),
    enforcer: Annotated[MACIEnforcer | None, Depends(_get_maci_enforcer)] = None,
) -> MACIActionValidationResponse:
