    ...
    **kwargs: Any,
    ...
    async with _singleton_lock:
        if _redis_client is None:
            del kwargs
            client = CircuitBreakerRedisClient(redis_url=redis_url)
            await client.initialize()
...
async def get_all_circuit_health() -> JSONDict:
    ...
    client_health: dict[str, JSONDict] = {}
...
async def _single_client_health_response(
    client: _HealthClientProtocol | None,
    name: str,
) -> tuple[JSONDict, int]:
...
async def _kafka_flush_response() -> tuple[JSONDict, int]:
...
def create_circuit_breaker_client_router() -> object | None:
    ...
    try:
        ...
        @router.get("")
        async def get_all_clients_health() -> Any:
