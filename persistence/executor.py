...
ActivityCallable: TypeAlias = Callable[["WorkflowContext", JSONDict], Awaitable[object]]
CompensationCallable: TypeAlias = Callable[["WorkflowContext", JSONDict], Awaitable[object]]
...
class WorkflowContext:
    ...
    def __init__(
        ...
        self._compensation_handlers: dict[str, CompensationCallable] = {}

    @property
    def workflow_id(self) -> str:
        return str(self.workflow_instance.workflow_id)

    @property
    def tenant_id(self) -> str:
        return str(self.workflow_instance.tenant_id)

    @property
    def input(self) -> JSONDict | None:
        return cast(JSONDict | None, self.workflow_instance.input)
...
class DurableWorkflowExecutor:
    ...
    def __init__(
        ...
        self._workflows: dict[str, WorkflowCallable] = {}

    def workflow(self, name: str) -> Callable[[WorkflowCallable], WorkflowCallable]:
        """Decorator to register a workflow."""

        def decorator(func: WorkflowCallable) -> WorkflowCallable:
    ...
    async def _finalize_workflow_success(
        self, instance: WorkflowInstance, result: object, span: _SpanProtocol | None
    ) -> None:
        """Record completed workflow state, metrics, and close span."""
        instance.status = WorkflowStatus.COMPLETED
        instance.output = _coerce_json_dict(result)
    ...
    async def _finalize_workflow_cancelled(
        self, instance: WorkflowInstance, exc: BaseException, span: _SpanProtocol | None
    ) -> None:
    ...
    async def _get_cached_step_result(
        ...
        if existing and existing.status == StepStatus.COMPLETED:
            ...
            if isinstance(existing.output, dict):
                await workflow_cache.set_step_result(
                    context.workflow_id, idempotency_key, cast(JSONDict, existing.output)
                )
    ...
    async def execute_activity(
        ...
        if last_error is None:
            raise RuntimeError(f"Step {step_name} failed without an exception")
    ...
    async def _record_event(
        self, workflow_instance_id: UUID, event_type: EventType, event_data: JSONDict
    ) -> None:
