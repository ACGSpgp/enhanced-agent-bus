...
class XAIAdapter(OpenAIAdapter):
    ...
    CACHED_PRICING: ClassVar[dict[str, float]] = {
        "grok-4-1-fast": 0.05,
    ...
    def _estimate_cost_from_usage(self, usage: dict[str, Any]) -> CostEstimate:
        ...
        return self.estimate_cost(
            ...
            cached_tokens=int(cached_tokens) if isinstance(cached_tokens, (int, float)) else 0,
            reasoning_tokens=(
                int(reasoning_tokens) if isinstance(reasoning_tokens, (int, float)) else 0
            ),
        )
