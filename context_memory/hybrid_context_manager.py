...
class SharedAttentionProcessor:
    ...
    def forward(
        ...
        if TORCH_AVAILABLE and isinstance(x, torch.Tensor):
            mask = attention_mask if isinstance(attention_mask, torch.Tensor) else None
            return self._forward_torch(x, mask, critical_positions)
        ...
        if ssm_output is not None and attn_output is not None:
            if TORCH_AVAILABLE:
                ...
                combined_output = torch.cat(
                    [cast(Any, attn_output), cast(Any, ssm_output)],
                    dim=1,
                )
            elif NUMPY_AVAILABLE:
                import numpy as np
