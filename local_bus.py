...
class LocalEventBus:
    ...
    def __init__(self) -> None:
        ...
        self._queues: dict[str, dict[str, list[asyncio.Queue[JSONDict]]]] = defaultdict(
    ...
    async def subscribe(
        self,
        tenant_id: str | Callable[[JSONDict], Any],
        message_types: list[MessageType] | None = None,
        handler: Callable[[JSONDict], Any] | None = None,
    ) -> None:
        ...
        if handler is None:
            raise ValueError("handler is required for subscription")

        for mt in message_types or []:
            q: asyncio.Queue[JSONDict] = asyncio.Queue()
    ...
    async def _consume_queue(
        self, q: asyncio.Queue[JSONDict], handler: Callable[[JSONDict], Any]
    ) -> None:
