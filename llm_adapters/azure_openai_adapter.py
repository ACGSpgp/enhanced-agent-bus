...
class AzureOpenAIAdapter(BaseLLMAdapter):
    ...
    MODEL_PRICING: ClassVar[dict[str, dict[str, float]]] = {
        "gpt-5.5": {"prompt": 2.00, "completion": 16.00},
    ...
    def _get_client(self) -> _AzureOpenAIClientProtocol:
        ...
        if self._client is None:
            ...
            self._client = cast(
                ...
                cast(Callable[..., object], AzureOpenAI)(**client_kwargs),
            )
    ...
    def _get_async_client(self) -> _AzureOpenAIClientProtocol:
        ...
        if self._async_client is None:
            ...
            self._async_client = cast(
                _AzureOpenAIClientProtocol,
                cast(Callable[..., object], AsyncAzureOpenAI)(**client_kwargs),
            )
    ...
    def estimate_cost(
        ...
        # Default to GPT-5.5 pricing if model not found
        if pricing is None:
            logger.warning(f"Pricing not found for model {self.model}, using GPT-5.5 pricing")
            pricing = self.MODEL_PRICING["gpt-5.5"]
