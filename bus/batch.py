...
class BatchProcessor:
    ...
    async def process_batch(self, batch_request: BatchRequest) -> BatchResponse:
        ...
        normalized_batch_tenant = normalize_tenant_id(batch_request.tenant_id)
        if normalized_batch_tenant is not None:
            batch_request.tenant_id = normalized_batch_tenant
        for item in batch_request.items:
            normalized_item_tenant = normalize_tenant_id(item.tenant_id)
            if normalized_item_tenant is not None:
                item.tenant_id = normalized_item_tenant
        ...
        batch_processor = BatchMessageProcessor(
            message_processor=self._processor,
