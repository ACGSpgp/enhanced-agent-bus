...
class DatabaseTenantRepository:
    ...
    def _orm_to_pydantic(self, orm: TenantORM) -> Tenant:
        ...
        orm_status: str | None
        if isinstance(orm.status, TenantStatusEnum):
            orm_status = orm.status.value
        elif orm.status:
            orm_status = str(orm.status)
        else:
            orm_status = None
        return Tenant(
            ...
            name=str(orm.name),
            slug=str(orm.slug),
            status=status_map.get(orm_status or "", TenantStatus.PENDING),
    ...
    async def create_tenants_bulk(self, tenants_data: list[JSONDict]) -> list[Tenant]:
    ...
    async def get_tenant(self, tenant_id: str) -> Tenant | None:
        ...
        orm = cast(Any, result.scalar_one_or_none())
        if not orm:
            return None
    ...
    async def activate_tenant(self, tenant_id: str) -> Tenant | None:
        ...
        now = datetime.now(UTC)
        orm.status = TenantStatusEnum.ACTIVE
        orm.activated_at = now
        orm.updated_at = now
    ...
    async def suspend_tenant(
        ...
        now = datetime.now(UTC)
        orm.status = TenantStatusEnum.SUSPENDED
        orm.suspended_at = now
        orm.updated_at = now

        if reason:
            metadata: JSONDict = orm.metadata_ or {}
    ...
    async def delete_tenant(self, tenant_id: str) -> bool:
        ...
        orm.status = TenantStatusEnum.DEACTIVATED
        orm.updated_at = datetime.now(UTC)
