...
class CircuitBreakerKafkaProducer:
    ...
    async def initialize(self) -> None:
        ...
        self._circuit_breaker = cast(
            Any, await get_service_circuit_breaker("kafka_producer", config)
        )
    ...
    async def close(self) -> None:
        ...
        if self._producer:
            await cast(Any, self._producer).flush()
    ...
    async def _send_raw(self, topic: str, value: JSONDict, key: bytes | None) -> None:
        ...
        if self._producer:
            await cast(Any, self._producer).send_and_wait(topic, value=value, key=key)
        else:
    ...
    async def send(
        ...
        try:
            await cast(Any, self._producer).send_and_wait(topic, value=value, key=key_bytes)
