...
class TestStabilizeWeights:
    ...
    def test_rust_false_sinkhorn_none_torch_false(self) -> None:
        ...
        saved = (m.RUST_AVAILABLE, m._sinkhorn_projection, m.TORCH_AVAILABLE)
        try:
            m.RUST_AVAILABLE = False
            m._sinkhorn_projection = None
            m.TORCH_AVAILABLE = False
            assert sess._stabilize_weights() == {"a1": 1.5, "a2": 0.5}
        finally:
            m.RUST_AVAILABLE, m._sinkhorn_projection, m.TORCH_AVAILABLE = saved
    ...
    def test_rust_false_sinkhorn_present_torch_true_hits_line_197(self) -> None:
        ...
        try:
            m.RUST_AVAILABLE = False
            m._sinkhorn_projection = MagicMock()
    ...
    def test_rust_true_numpy_false_raises_import_error(self) -> None:
        ...
        m = self._get_vc_module()
        sess = _session(agent_weights={"a1": 2.0})
        saved = (m.RUST_AVAILABLE, m.NUMPY_AVAILABLE, m._sinkhorn_projection, m.TORCH_AVAILABLE)
        try:
            ...
            m.RUST_AVAILABLE, m.NUMPY_AVAILABLE, m._sinkhorn_projection, m.TORCH_AVAILABLE = saved
    ...
    def test_rust_true_sinkhorn_raises_value_error(self) -> None:
        ...
        saved = (
            ...
            m._rust_opt,
            m._np,
            m._sinkhorn_projection,
            m.TORCH_AVAILABLE,
        )
        try:
            m.RUST_AVAILABLE = True
            m.NUMPY_AVAILABLE = True
            m._rust_opt = mock_rust
            m._np = mock_np
            m._sinkhorn_projection = None
            m.TORCH_AVAILABLE = False
    ...
    def test_rust_success_nonzero_total_normalizes(self) -> None:
        ...
        saved = (m.RUST_AVAILABLE, m.NUMPY_AVAILABLE, m._rust_opt, m._np)
        try:
            m.RUST_AVAILABLE = True
            m.NUMPY_AVAILABLE = True
            m._rust_opt = mock_rust
            ...
            m.RUST_AVAILABLE, m.NUMPY_AVAILABLE, m._rust_opt, m._np = saved
