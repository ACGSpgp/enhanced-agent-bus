...
from typing import Any, TypeAlias, TypedDict, TypeVar, Unpack, cast

try:
    from enhanced_agent_bus._compat.types import JSONDict as ImportedJSONDict
    from enhanced_agent_bus._compat.types import JSONList as ImportedJSONList
except ImportError:
    ImportedJSONDict = dict[str, Any]
    ImportedJSONList = list[Any]

JSONDict: TypeAlias = ImportedJSONDict
JSONList: TypeAlias = ImportedJSONList
...
ToolHandlerFunc = Callable[..., object]
F = TypeVar("F", bound=ToolHandlerFunc)
...
class ToolParameter:
    ...
    def to_json_schema(self) -> JSONDict:
        ...
        mapped_type = type_mapping.get(self.type.lower(), "string")
        ...
        return sanitize_schema_fragment(schema)
...
class ToolDefinition:
    ...
    handler: ToolHandlerFunc | None = None
...
class ToolDecoratorOptions(TypedDict, total=False):
    return_type: str
...
def tool(
    ...
    returns: str = "",
    **kwargs: Unpack[ToolDecoratorOptions],
) -> Callable[[F], F]:
    """
