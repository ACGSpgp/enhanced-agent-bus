...
if TYPE_CHECKING:
    ...
    try:
        from enhanced_agent_bus._compat.types import JSONList as _JSONList
    except ImportError:
        _JSONList = list
...
class DefaultDeliberationActivities:
    ...
    async def calculate_impact_score(
        ...
        try:
            ...
            return scorer.calculate_impact_score({"content": content}, context)
        except ImportError:
...
class DeliberationWorkflow:
    ...
    def _register_compensation(self, compensation: Callable[[], Awaitable[None]]) -> None:
    ...
    async def _collect_approvals(
        ...
        decision = human_result.get("decision")
        reviewer = human_result.get("reviewer")
        self._human_decision = decision if isinstance(decision, str) else None
        self._human_reviewer = reviewer if isinstance(reviewer, str) else None
        return consensus_reached

    async def _execute_compensations(self) -> list[str]:
    ...
    def signal_human_decision(self, decision: str, reviewer: str) -> None:
    ...
    def signal_vote(self, vote: Vote) -> None:
