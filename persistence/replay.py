...
class ReplayEngine:
        ...
        for event in events:
            if event.event_type == EventType.WORKFLOW_STARTED:
                instance = WorkflowInstance(
                    ...
                    workflow_type=cast(str, event.event_data.get("workflow_type", "")),
                    workflow_id=cast(
                        str,
                        event.event_data.get("workflow_id", str(workflow_instance_id)),
                    ),
                    tenant_id=cast(str, event.event_data.get("tenant_id", "default")),
                    input=cast(dict[str, object] | None, event.event_data.get("input")),
                ...
                if instance:
                    ...
                    instance.output = cast(dict[str, object] | None, event.event_data.get("output"))

            elif event.event_type == EventType.WORKFLOW_FAILED:
                if instance:
                    instance.status = WorkflowStatus.FAILED
                    instance.error = cast(str | None, event.event_data.get("error"))

            elif event.event_type == EventType.WORKFLOW_CANCELLED:
                if instance:
                    instance.status = WorkflowStatus.CANCELLED
                    instance.error = cast(str | None, event.event_data.get("reason"))
    ...
    async def get_replay_timeline(self, workflow_instance_id: UUID) -> list[dict[str, Any]]:
        """
