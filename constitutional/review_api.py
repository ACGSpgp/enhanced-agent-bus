...
try:
    ...
    from .rollback_engine import (
        RollbackReason as _RollbackReasonImport,
    )
    from .rollback_engine import (
        RollbackSagaActivities,
    )
    from .rollback_engine import (
        rollback_amendment as _RollbackAmendmentImport,
    )

    ROLLBACK_AVAILABLE = True
    _ImportedRollbackReason = _RollbackReasonImport
    _imported_rollback_amendment = _RollbackAmendmentImport
except ImportError:
    # Fallback for standalone usage
    ROLLBACK_AVAILABLE = False
    _ImportedRollbackReason = cast(Any, None)
    _imported_rollback_amendment = cast(Any, None)

RollbackReason: Any = _ImportedRollbackReason
rollback_amendment: Any = _imported_rollback_amendment
...
def _typed_route(
    method_name: str, *args: object, **kwargs: object
) -> Callable[[_RouteHandler], _RouteHandler]:
    route_factory = cast(
        Callable[..., Callable[[_RouteHandler], _RouteHandler]],
        getattr(router, method_name),
    )
    return route_factory(*args, **kwargs)
...
def _review_maci_action(action_name: str) -> Any:
...
class AmendmentListQuery(_PydanticBaseModel):
...
class AmendmentListResponse(_PydanticBaseModel):
...
class AmendmentDetailResponse(_PydanticBaseModel):
...
class ApprovalRequest(_PydanticBaseModel):
...
class RejectionRequest(_PydanticBaseModel):
...
class ApprovalResponse(_PydanticBaseModel):
...
class RollbackRequest(_PydanticBaseModel):
...
class RollbackResponse(_PydanticBaseModel):
...
async def list_amendments(
    ...
    try:
        ...
        if order_by == "impact_score":
            paginated_amendments = sorted(
                paginated_amendments,
                key=lambda amendment: float(getattr(amendment, "impact_score", 0.0) or 0.0),
                reverse=reverse_sort,
            )
        elif order_by == "status":
            paginated_amendments = sorted(
                paginated_amendments,
                key=lambda amendment: str(getattr(amendment, "status", "")),
                reverse=reverse_sort,
            )
        else:
            paginated_amendments = sorted(
                paginated_amendments,
                key=lambda amendment: str(getattr(amendment, "created_at", "")),
                reverse=reverse_sort,
            )
...
async def approve_amendment(
    ...
    try:
        ...
        if dependencies.hitl is None:
            raise RuntimeError("HITL integration is required for approval processing")
        ...
        await dependencies.audit_client.log_event(
...
async def rollback_to_version(
    ...
    try:
        ...
        try:
            ...
            await storage.save_version(current_version)
