...
class MiniCPMSemanticScorer(BaseScoringAlgorithm):
    ...
    def __init__(self, config: MiniCPMScorerConfig | None = None) -> None:
    ...
    def _compute_domain_embeddings(self) -> None:
        ...
        try:
            ...
            for domain in GovernanceDomain:
                ...
                if provider is None:
                    return
                embeddings = provider.embed_batch(reference_texts)
    ...
    def _get_embedding(self, text: str) -> list[float] | None:
        ...
        try:
            ...
            self._embedding_cache[cache_key] = cast(list[float], embedding)
    ...
    def _extract_direct_content(self, context: dict[str, Any]) -> list[str]:
    ...
    def _extract_message_content(self, context: dict[str, Any]) -> list[str]:
    ...
    def _extract_action_reasoning(self, context: dict[str, Any]) -> list[str]:
    ...
    def _extract_tools_content(self, context: dict[str, Any]) -> list[str]:
    ...
    def score(self, context: dict[str, Any]) -> ScoringResult:
        """
        ...
        for domain in GovernanceDomain:
            if use_semantic:
                ...
                semantic_score = self._calculate_semantic_similarity(
                    cast(list[float], text_embedding), domain
                )
            else:
                semantic_score = 0.0
    ...
    def score_batch(self, contexts: list[dict[str, Any]]) -> list[ScoringResult]:
        """
        ...
        if self._provider_available and self._provider is not None:
            try:
                ...
                if non_empty_texts:
                    ...
                    provider = self._provider
                    if provider is None:
                        raise RuntimeError("MiniCPM provider unavailable during batch scoring")
                    embeddings = provider.embed_batch(list(batch_texts))
