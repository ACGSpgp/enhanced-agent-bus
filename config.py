...
def _load_litellm_cache_class() -> Any:
    """Load LiteLLM's cache class without relying on re-exported stub metadata."""
    ...
    def from_environment(cls) -> "BusConfiguration":
        ...
        if config.llm_use_cache and not os.getenv("PYTEST_CURRENT_TEST"):
            try:
                ...
                cache_cls = _load_litellm_cache_class()
                litellm.cache = cache_cls(
                    type="redis",
...
settings: object
try:
    from enhanced_agent_bus._compat.config import settings as _compat_settings
except ImportError:
    settings = BusConfiguration()
else:
