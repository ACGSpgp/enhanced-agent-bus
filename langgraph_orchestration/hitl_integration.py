...
class InMemoryHITLHandler:
    ...
    async def request_human_input(
        ...
        if self.auto_response:
            ...
            self.response_history.append(auto_response)
        ...
        response: HITLResponse | None = self.responses.get(request.id)
        if response is None:
...
class HITLInterruptHandler:
    ...
    def __init__(
        ...
        checkpoint_manager: _CheckpointManagerProtocol | None = None,
        constitutional_hash: str = CONSTITUTIONAL_HASH,
    ) -> None:
    ...
    async def create_interrupt(
        ...
        if self.checkpoint_manager:
            checkpoint_obj = await self.checkpoint_manager.create_checkpoint(
                context=context,
                node_id=node_id,
                state=state,
                metadata={"interrupt_type": interrupt_type.value, "reason": reason},
            )
            checkpoint_id = getattr(checkpoint_obj, "id", None)
