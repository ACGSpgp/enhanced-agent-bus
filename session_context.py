...
class SessionContextStore:
    ...
    async def connect(self) -> bool:
        ...
        try:
            redis_client = cast(
                _AsyncRedisProtocol,
                aioredis.from_url(self.redis_url, encoding="utf-8", decode_responses=True),
            )
            # Test connection
...
class SessionContextManager:
    ...
    async def _get_internal(self, session_id: str, tenant_id: str) -> SessionContext | None:
        ...
        try:
            ...
            stored_session_context = await self.store.get(session_id, tenant_id)

            if stored_session_context:
                self._update_cache(stored_session_context)
