...
class BatchRequest(BaseModel):
    ...
    def timeout_ms(self) -> int:
        ...
        model_extra = self.model_extra or {}
        timeout_value = model_extra.get("timeout_ms")
        if timeout_value is None:
            timeout_value = self.options.get("timeout_ms", 30000)
    ...
    def fail_fast(self) -> bool:
        ...
        model_extra = self.model_extra or {}
        return bool(model_extra.get("fail_fast") or self.options.get("fail_fast", False))
    ...
    def deduplicate(self) -> bool:
        ...
        model_extra = self.model_extra or {}
        val = model_extra.get("deduplicate")
        if val is not None:
            return bool(val)
