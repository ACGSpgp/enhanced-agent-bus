...
from ...pqc_enforcement_config import EnforcementModeConfigService, StorageUnavailableError
from ...pqc_enforcement_models import EnforcementModeRequest, EnforcementModeResponse
...
if TYPE_CHECKING:
    ...
    try:
        from enhanced_agent_bus._compat.security.auth import get_current_user as _get_current_user
    except ImportError:  # pragma: no cover - fallback for isolated test runs

        async def _get_current_user(
            credentials: HTTPAuthorizationCredentials = cast(HTTPAuthorizationCredentials, ...),
        ) -> Any:
            del credentials
            raise HTTPException(status_code=401, detail="Auth not configured")

    get_current_user = cast(Any, _get_current_user)
