...
class TestLifespanStartup:
    ...
    async def test_startup_attaches_pg_fallback_to_pqc_service(self):
        ...
        with (
            patch.object(app_module, "MessageProcessor", return_value=fake_mp),
            patch.object(app_module, "BatchMessageProcessor", return_value=fake_batch),
            # Patch at the workflow-components level to bypass the isinstance(repo, class)
            # check that would fail when PostgresWorkflowRepository is replaced by a mock.
            patch.object(
                app_module,
                "_initialize_workflow_components",
                return_value=(fake_executor, fake_repo),
            ),
...
class TestLifespanShutdown:
    ...
    async def _run_full_lifespan(self, fake_app, extra_patches=None, fake_sessions_return=True):
        ...
        fake_mp = MagicMock()
        fake_batch = MagicMock()
        # Use spec so isinstance(fake_repo, PostgresWorkflowRepository) is True at shutdown.
        fake_repo = AsyncMock(spec=app_module.PostgresWorkflowRepository)
        fake_repo.initialize = AsyncMock()
        fake_repo.close = AsyncMock()
        mock_logger = MagicMock()
        ...
        ctx_patches = [
            ...
            patch.object(app_module, "BatchMessageProcessor", return_value=fake_batch),
            # Patch at the workflow-components level to bypass the isinstance(repo, class)
            # check that would fail when PostgresWorkflowRepository is a MagicMock.
            patch.object(
                app_module,
                "_initialize_workflow_components",
                return_value=(MagicMock(), fake_repo),
            ),
            patch.object(app_module, "logger", mock_logger),
            patch.dict(
                "os.environ",
    ...
    async def test_shutdown_with_cache_warming_active(self):
        ...
        # Patch _stop_cache_warmer_if_running directly: the lifespan calls it on shutdown,
        # and the internal import `from .. import CACHE_WARMING_AVAILABLE, get_cache_warmer`
        # is not reliably patchable in the pytest import graph.
        async def _simulated_stop():
            if fake_warmer.is_warming:
                fake_warmer.cancel()
    ...
    async def test_shutdown_with_cache_warming_not_available(self):
        ...
        fake_app = FastAPI()
        # Default path: cache warming not available — _stop_cache_warmer_if_running runs
        # with the real module values, no cancel triggered. Just verify no exception.
