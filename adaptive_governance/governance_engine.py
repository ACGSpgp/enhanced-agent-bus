...
if TYPE_CHECKING:
    from enhanced_agent_bus._compat.types import JSONDict, MessagePayload, PolicyContext
    from enhanced_agent_bus.config import BusConfiguration
else:
    try:
        from enhanced_agent_bus._compat.types import JSONDict as _JSONDict
        from enhanced_agent_bus._compat.types import MessagePayload as _MessagePayload
        from enhanced_agent_bus._compat.types import PolicyContext as _PolicyContext
    except ImportError:
        _JSONDict = dict[str, Any]
        _MessagePayload = dict[str, Any]
        _PolicyContext = dict[str, Any]
...
if available("feedback_handler"):
    ...
    FeedbackEvent: Any = cast(Any, _feedback_handler_module).FeedbackEvent
    FeedbackHandler: Any = cast(Any, _feedback_handler_module).FeedbackHandler
    FeedbackType: Any = cast(Any, _feedback_handler_module).FeedbackType
    OutcomeStatus: Any = cast(Any, _feedback_handler_module).OutcomeStatus
    get_feedback_handler: Any = cast(Any, _feedback_handler_module).get_feedback_handler
...
if available("drift_monitoring"):
    ...
    DriftDetector: Any = cast(Any, _drift_module).DriftDetector
    DriftReport: Any = cast(Any, _drift_module).DriftReport
    DriftSeverity: Any = cast(Any, _drift_module).DriftSeverity
    DriftStatus: Any = cast(Any, _drift_module).DriftStatus
    get_drift_detector: Any = cast(Any, _drift_module).get_drift_detector
...
if available("online_learning"):
    ...
    LearningResult: Any = cast(Any, _online_learning_module).LearningResult
    LearningStatus: Any = cast(Any, _online_learning_module).LearningStatus
    ModelType: Any = cast(Any, _online_learning_module).ModelType
    OnlineLearningPipeline: Any = cast(Any, _online_learning_module).OnlineLearningPipeline
    PredictionResult: Any = cast(Any, _online_learning_module).PredictionResult
    get_online_learning_pipeline: Any = cast(
        Any, _online_learning_module
...
if available("anomaly_monitoring"):
    AnomalyMonitor: Any = cast(Any, import_module(require("anomaly_monitoring"))).AnomalyMonitor
...
if available("ab_testing"):
    ...
    AB_TEST_SPLIT = cast(Any, _ab_testing_module).AB_TEST_SPLIT
    ABTestRouter: Any = cast(Any, _ab_testing_module).ABTestRouter
    CohortType: Any = cast(Any, _ab_testing_module).CohortType
    MetricsComparison: Any = cast(Any, _ab_testing_module).MetricsComparison
    PromotionResult: Any = cast(Any, _ab_testing_module).PromotionResult
    RoutingResult: Any = cast(Any, _ab_testing_module).RoutingResult
    ShadowPolicyExecutor = getattr(_ab_testing_module, "ShadowPolicyExecutor", None)
    get_ab_test_router: Any = cast(Any, _ab_testing_module).get_ab_test_router
...
class AdaptiveGovernanceEngine:
    ...
    def __init__(self, constitutional_hash: str, config: "BusConfiguration | None" = None):
        ...
        self._feedback_handler: Any | None = None
        ...
        self.learning_thread: asyncio.Task[None] | None = None  # Legacy alias
        self.running = False
        ...
        self._latest_drift_report: Any | None = None
        self._initialize_drift_detector()

        self._river_feature_names = self._default_river_feature_names()
        self.river_model: Any | None = None
        self._initialize_river_model()

        self._ab_test_router: Any | None = None
        self._shadow_executor: Any | None = None
        self._initialize_ab_test_router()

        self._anomaly_monitor: Any | None = None
    ...
    def _initialize_ab_test_router(self) -> None:
        ...
        try:
            ...
            if ShadowPolicyExecutor is not None:
    ...
    def _initialize_anomaly_monitor(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to initialize anomaly monitor: {e}")

    @property
    def _learning_thread(self) -> asyncio.Task[None] | None:
    ...
    def _record_ab_test_request(
        ...
        if self._ab_test_router is None:
            return
    ...
    def get_ab_test_router(self) -> Any | None:
    ...
    def get_ab_test_metrics(self) -> JSONDict | None:
        ...
        try:
            return cast(JSONDict, self._ab_test_router.get_metrics_summary())
        except (RuntimeError, ValueError, TypeError) as e:
            logger.warning(f"Failed to get A/B test metrics: {e}")
            return None

    def get_ab_test_comparison(self) -> Any | None:
    ...
    def promote_candidate_model(self, force: bool = False) -> Any | None:
    ...
    def _collect_drift_data(self) -> Any | None:
        """Collect recent decision data for drift analysis."""
        try:
