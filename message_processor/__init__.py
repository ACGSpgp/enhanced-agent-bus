...
if TYPE_CHECKING:
    from enhanced_agent_bus._compat.types import JSONDict, JSONValue
else:
...
if _metering_integration is not None:
    get_metering_hooks = _metering_integration.get_metering_hooks
else:

    def get_metering_hooks(config: object | None = None) -> object | None:
        _ = config
        return None
...
if _opa_client_module is not None:
    get_opa_client = _opa_client_module.get_opa_client
else:

    def get_opa_client(fail_closed: bool = True) -> object | None:
        _ = fail_closed
        return None
...
if _opa_client_module is not None:
    ...
    _ImportedOPAClient = None


_OPAClient: Any = _ImportedOPAClient
...
def _typed_timed(metric_name: str) -> Callable[[F], F]:
    return timed(metric_name)
...
class MessageProcessor:
    ...
    _session_context_manager: SessionContextManager | None
    _session_resolver: SessionContextResolver
    _session_coordinator: SessionCoordinator
    _security_scanner: MessageSecurityScanner
    _gate_coordinator: GateCoordinator
    _verification_orchestrator: VerificationOrchestrator
    _governance_coordinator: GovernanceCoordinator
    _result_finalizer: ResultFinalizer
    _verification_coordinator: VerificationCoordinator
    ...
    def __init__(self, **kwargs: object) -> None:
        ...
        self._policy_fail_closed = bool(
            kwargs.get("policy_fail_closed", self.config.policy_fail_closed)
        )
        self._use_rust = bool(kwargs.get("use_rust", True))
        self._enable_metering = bool(kwargs.get("enable_metering", True))
        ...
        self._enable_maci = bool(kwargs.get("enable_maci", True)) and not self._isolated_mode
        self._maci_registry, self._maci_enforcer, self._maci_strict_mode = (
            kwargs.get("maci_registry"),
            kwargs.get("maci_enforcer"),
            bool(kwargs.get("maci_strict_mode", True)),
        )
        ...
        self._validation_cache: Any = LRUCache(maxsize=DEFAULT_LRU_CACHE_SIZE)

        # SDPC Phase 2/3 Verifiers
        from enhanced_agent_bus.builder import build_pqc_service, build_sdpc_verifiers

        sdpc = cast(Any, build_sdpc_verifiers(self.config))
        ...
        self._independent_validator_threshold = float(
            ...
            else IMPACT_DELIBERATION_THRESHOLD
        )
        self._agent_workflow_metrics = (
            get_agent_workflow_metrics_collector() if AGENT_WORKFLOW_METRICS_AVAILABLE else None
        )
        self._session_resolver = cast(
            SessionContextResolver,
            kwargs.get("session_resolver")
            or SessionCoordinator.build_session_resolver(
                self.config,
                self._session_context_manager,
            ),
        )
        self._session_coordinator = cast(
            SessionCoordinator,
            kwargs.get("session_coordinator")
            or SessionCoordinator(
        ...
        self._security_scanner = cast(
            MessageSecurityScanner,
            kwargs.get("security_scanner") or MessageSecurityScanner(),
        )
        self._gate_coordinator = cast(
            GateCoordinator,
            kwargs.get("gate_coordinator")
            or GateCoordinator(
        ...
        self._governance_coordinator = cast(
            GovernanceCoordinator,
            kwargs.get("governance_coordinator")
        ...
        self._result_finalizer = cast(
            ResultFinalizer,
            kwargs.get("result_finalizer") or ResultFinalizer(),
        )
        self._verification_coordinator = cast(
            VerificationCoordinator,
            kwargs.get("verification_coordinator")
            or VerificationCoordinator(
                ...
                handlers=cast(JSONDict, self._handlers),
    ...
    def _initialize_opa_client(self) -> object | None:
        if self._isolated_mode:
            return None
        try:
            return cast(object | None, get_opa_client())
        except Exception:
            if _OPAClient is not None:
                try:
                    return cast(object | None, _OPAClient())
                except Exception:
    ...
    def _build_verification_orchestrator(
        ...
        return cast(VerificationOrchestrator, verification_orchestrator)
    ...
    def _apply_verification_runtime_dependencies_legacy(
        ...
        orchestrator = cast(Any, verification_orchestrator)
        orchestrator.intent_classifier = runtime_dependencies.intent_classifier
        orchestrator.asc_verifier = runtime_dependencies.asc_verifier
        orchestrator.graph_check = runtime_dependencies.graph_check
        orchestrator.pacar_verifier = runtime_dependencies.pacar_verifier
        orchestrator.evolution_controller = runtime_dependencies.evolution_controller
        orchestrator.ampo_engine = runtime_dependencies.ampo_engine
        orchestrator._IntentType = runtime_dependencies.intent_type
        orchestrator._enable_pqc = runtime_dependencies.enable_pqc
        orchestrator._pqc_service = runtime_dependencies.pqc_service
        orchestrator._pqc_config = runtime_dependencies.pqc_config
    ...
    @_typed_timed("extract_session_context")
    ...
    def _requires_independent_validation(self, msg: AgentMessage) -> bool:
        ...
        self._sync_coordinator_runtime()
        return bool(self._gate_coordinator.requires_independent_validation(msg))
    ...
    def _extract_message_session_id(self, msg: AgentMessage) -> str | None:
        ...
        return extract_session_id_for_pacar(msg)

    @_typed_timed("message_process")
        ...
        for attempt in range(max_retries):
            try:
                if CIRCUIT_BREAKER_ENABLED:
                    return cast(
                        ValidationResult,
                        await cast(Any, self._process_cb).call(self._do_process, msg),
                    )
    ...
    @_typed_timed("message_do_process")
    ...
    async def _handle_successful_processing(
        ...
        def schedule_governance_audit_event(
            ...
            self._result_finalizer.schedule_governance_audit_event(
                ...
                result=audit_result,
                audit_client=self._audit_client,
                schedule_background_task_fn=cast(Any, schedule_background_task),
                background_tasks=cast(set[object], self._background_tasks),
    ...
    async def _async_metering_callback(self, msg: AgentMessage, lat: float) -> None:
        ...
        try:
            cast(Any, self._metering_hooks).on_constitutional_validation(
    ...
    async def _get_dlq_redis(self) -> object:
        ...
        self._dlq_redis = aioredis.from_url(str(redis_url), decode_responses=True)
        return self._dlq_redis
    ...
    def _detect_prompt_injection(self, msg: AgentMessage) -> ValidationResult | None:
        ...
        return self._security_scanner.detect_prompt_injection(msg)
    ...
    def _apply_metrics_enrichment(self, metrics: JSONDict) -> None:
        ...
        self._sync_runtime_state_from_coordinators()
        enrich_metrics_with_opa_stats(metrics, cast(Any, self._opa_client))
        collector = getattr(self, "_agent_workflow_metrics", None)
    ...
    async def initialize_mcp(self, config: object) -> None:
        ...
        if isinstance(config, dict):
            try:
                mcp_config = cast(Any, MCPConfig)(**config)
            except (TypeError, ValueError) as exc:
                logger.error("mcp_initialize_invalid_config_dict", error=str(exc))
                return
        elif MCPConfig is not None and isinstance(config, MCPConfig):
            mcp_config = config
        else:
            logger.error(
        ...
        for srv in mcp_config.servers:
            ...
            client_cfg = cast(Any, MCPClientConfig)(
                server_url=server_url,
                server_id=srv.name,
                call_timeout=srv.timeout,
                metadata={"transport": srv.transport},
            )
            client = cast(Any, MCPClient)(config=client_cfg)
    ...
    async def handle_tool_request(
        ...
        if self._mcp_pool is None:
            ...
            return cast(Any, MCPToolResult).error_result(
                tool_name,
        ...
        if self._maci_registry is not None:
            try:
                agent_record = await cast(Any, self._maci_registry).get_agent(agent_id)
