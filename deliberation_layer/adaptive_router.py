...
_LazyImports: TypeAlias = dict[str, object]
_RoutingDecision: TypeAlias = dict[str, Any]
_PerformanceMetrics: TypeAlias = dict[str, float]


# Lazy imports to avoid circular dependency
def _get_imports() -> _LazyImports:
    ...
    def get_deliberation_queue() -> object:
...
_imports_cache: _LazyImports | None = None
...
def get_deliberation_queue() -> object:
    ...
    queue_factory = cast(Any, _lazy_import("get_deliberation_queue"))
...
class AdaptiveRouter:
    ...
    def __init__(
        ...
        Args:
            ...
            enable_learning (bool): Whether to enable adaptive threshold learning.
                Defaults to True.
        """
        BusConfiguration_class = cast(Any, _lazy_import("BusConfiguration"))
        if config:
            self.config = config
        ...
        # Learning data
        self.routing_history: list[_RoutingDecision] = []
        self.performance_metrics: _PerformanceMetrics = {
            "total_messages": 0,
            "fast_lane_count": 0,
        ...
        get_deliberation_queue_func = cast(Any, _lazy_import("get_deliberation_queue"))
        self.deliberation_queue = get_deliberation_queue_func()

        # Initialize Intent Classifier with config
        IntentClassifier_class = cast(Any, _lazy_import("IntentClassifier"))
    ...
    async def route_message(
        ...
        IntentType_enum = cast(Any, _lazy_import("IntentType"))
    ...
    async def _route_to_fast_lane(
        ...
        return cast(JSONDict, routing_decision)
    ...
    def _record_routing_history(
        self, message: AgentMessage, routing_decision: _RoutingDecision
    ) -> None:
        """Record routing decision for learning."""
        if not self.enable_learning:
            return

        history_entry: _RoutingDecision = {
    ...
    async def update_performance_feedback(
        ...
        routing_entry: _RoutingDecision | None = None
        ...
        # Update counters
        routing_decision = cast(_RoutingDecision, routing_entry["routing_decision"])
        if routing_decision["lane"] == "deliberation":
    ...
    def get_routing_stats(self) -> JSONDict:
        ...
        if total == 0:
            return cast(JSONDict, self.performance_metrics.copy())
