...
def _hash_to_record(data: dict[str, str]) -> AgentHealthRecord:
    ...
    return AgentHealthRecord(
        ...
        health_state=HealthState(data["health_state"]),
...
class AgentHealthStore:
    ...
    async def get_health_record(self, agent_id: AgentID) -> AgentHealthRecord | None:
        ...
        try:
            raw = await _await_maybe(self._redis.hgetall(_health_key(agent_id)))
    ...
    async def upsert_health_record(self, record: AgentHealthRecord) -> None:
        ...
        try:
            ...
            await _await_maybe(self._redis.hset(key, mapping=mapping))
            await _await_maybe(self._redis.expire(key, HEALTH_RECORD_TTL_SECONDS))
            logger.info(
    ...
    async def get_override(self, agent_id: AgentID) -> HealingOverride | None:
        ...
        try:
            raw = await _await_maybe(self._redis.hgetall(_override_key(agent_id)))
            data: dict[str, str] = raw if isinstance(raw, dict) else {}
            if not data:
                return None
