...
class BedrockAdapter(BaseLLMAdapter):
    ...
    config: AWSBedrockAdapterConfig
    ...
    def _build_amazon_body(
        ...
        **kwargs: object,
        ...
        if stop:
            text_config = cast(dict[str, object], body["textGenerationConfig"])
            text_config["stopSequences"] = stop
    ...
    @staticmethod
    def _extract_anthropic_chunk_text(chunk_data: dict[str, object]) -> str | None:
        """Extract text from Anthropic chunk."""
        delta = chunk_data.get("delta", {})
        if isinstance(delta, dict) and delta.get("type") == "content_block_delta":
    ...
    @staticmethod
    def _extract_meta_chunk_text(chunk_data: dict[str, object]) -> str | None:
        """Extract text from Meta chunk."""
        text = chunk_data.get("generation", "")
        return text if isinstance(text, str) else None

    @staticmethod
    def _extract_amazon_chunk_text(chunk_data: dict[str, object]) -> str | None:
        """Extract text from Amazon chunk."""
        text = chunk_data.get("outputText", "")
        return text if isinstance(text, str) else None

    @staticmethod
    def _extract_generic_chunk_text(chunk_data: dict[str, object]) -> str | None:
        """Extract text from generic chunk."""
        text = chunk_data.get("text", chunk_data.get("completion", ""))
        return text if isinstance(text, str) else None
    ...
    async def _async_stream_with_client(
        self, async_client: Any, invoke_params: dict[str, object]
    ) -> AsyncIterator[str]:
