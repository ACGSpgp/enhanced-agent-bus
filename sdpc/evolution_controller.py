...
class EvolutionController:
    def __init__(self, failure_threshold: int = 3) -> None:
        ...
        self.dynamic_mutations: dict[str, list[str]] = {intent.value: [] for intent in IntentType}
    ...
    def _trigger_mutation(self, intent: IntentType) -> None:
        ...
        if self._mutation_guard is not None:
            try:
                ...
                logger.warning(
                    f"SDPC mutation blocked by invariant guard for {intent.value}: {exc}",
                )
                self.failure_history[intent.value] = 0
                return
    ...
    def reset_mutations(self, intent: IntentType | None = None) -> None:
        ...
        if self._mutation_guard is not None:
            ...
            try:
                ...
                logger.warning(f"SDPC reset_mutations blocked by guard: {exc}")
                return
