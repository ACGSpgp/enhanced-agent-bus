...
from enhanced_agent_bus._compat.security.jwt_algorithms import resolve_jwt_algorithm
...
_COLLABORATION_JWT_ALGORITHM = "HS256"


def _configured_collaboration_jwt_algorithm() -> str:
    """Resolve the collaboration JWT algorithm.

    CollaborationAPI validates bearer tokens with a shared secret
    (``COLLABORATION_SECRET_KEY``), so only HS256 is supported unless the module
    is extended to load asymmetric verification keys.
    """
    algorithm = resolve_jwt_algorithm(
        os.getenv("COLLABORATION_JWT_ALGORITHM"),
        error_code="COLLABORATION_JWT_ALGORITHM_INVALID",
        setting_name="COLLABORATION_JWT_ALGORITHM",
        default=_COLLABORATION_JWT_ALGORITHM,
    )
    if algorithm != _COLLABORATION_JWT_ALGORITHM:
        raise ValueError(
            "COLLABORATION_JWT_ALGORITHM must be 'HS256' because CollaborationAPI "
            f"uses a shared secret for JWT verification. Got {algorithm!r}."
        )
    return algorithm
...
class CollaborationAPI:
    ...
    def _validate_token(self, token: str) -> JSONDict | None:
        ...
        try:
            jwt_algorithm = _configured_collaboration_jwt_algorithm()
