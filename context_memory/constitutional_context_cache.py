...
class _RedisCacheProtocol(Protocol):
...
class ConstitutionalContextCache:
    ...
    def __init__(
        ...
        redis_client: _RedisCacheProtocol | None = None,
        constitutional_hash: str = CONSTITUTIONAL_HASH,
    ):
        self.config = config or CacheConfig()
        self.constitutional_hash = constitutional_hash
        self._redis_client: _RedisCacheProtocol | None = redis_client

        if constitutional_hash != CONSTITUTIONAL_HASH:
            raise ValueError(f"Invalid constitutional hash: {constitutional_hash}")

        # L1 cache (in-memory LRU)
        self._l1_cache: OrderedDict[str, CacheEntry[Any]] = OrderedDict()
    ...
    async def _set_to_l2(self, key: str, value: object, ttl: int) -> bool:
        ...
        try:
            ...
            await self._redis_client.setex(
                prefixed_key,
                ttl,
                json_dumps(serializable, default=str),
            )
            return True
        except L2_CACHE_OPERATION_ERRORS as e:
