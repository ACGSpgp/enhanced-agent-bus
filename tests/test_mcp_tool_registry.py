...
import enhanced_agent_bus.mcp_integration.tool_registry as tool_registry_module
...
class TestExecuteTool:
    ...
    @pytest.mark.asyncio
    async def test_execute_success_records_observation(self, monkeypatch):
        observations = []
        ...
        assert observations[0]["tool_type"] == tool.name
        assert observations[0]["success"] is True
        assert observations[0]["session_id"] == "sess-1"
    ...
    async def test_execute_failure_records_observation_error_type(self, monkeypatch):
        ...
        assert observations[0]["session_id"] == "sess-2"
        assert observations[0]["file_paths"] == ["src/input.py"]
