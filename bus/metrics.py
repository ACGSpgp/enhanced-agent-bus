...
class BusMetrics:
    ...
    async def get_metrics_async(self, policy_client: object | None) -> JSONDict:
        ...
        if policy_client:
            try:
                res = await cast(Any, policy_client).health_check()
    ...
    def get_metrics(self) -> JSONDict:
        ...
        processor = self._bus._processor
        if processor is not None and hasattr(processor, "get_metrics"):
            pm = cast(JSONDict, cast(Any, processor).get_metrics())
