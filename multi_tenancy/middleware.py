...
from .context import TenantContext, clear_tenant_context, set_current_tenant
...
def extract_tenant_from_request(request: Request) -> str | None:
    ...
    if hasattr(request.state, "user"):
        ...
        raw_jwt_tenant_id = getattr(user, "tenant_id", None)
        if raw_jwt_tenant_id is not None:
            jwt_tenant_id = str(raw_jwt_tenant_id)
        if jwt_tenant_id:
...
def extract_user_from_request(request: Request) -> str | None:
    ...
    if hasattr(request.state, "user"):
        raw_sub = getattr(request.state.user, "sub", None)
        if raw_sub is None:
            return None
        return str(raw_sub)
...
class TenantContextDependency:
    ...
    async def __call__(self, request: Request) -> TenantContext | None:
        ...
        if hasattr(request.state, "tenant_context"):
            return cast(TenantContext | None, request.state.tenant_context)
