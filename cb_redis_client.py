...
from .circuit_breaker import get_service_circuit_breaker
...
class CircuitBreakerRedisClient:
    ...
    async def close(self) -> None:
        ...
        if self._redis:
            try:
                ...
                close_coro = cast(Any, self._redis).close()
    ...
    async def get(self, key: str) -> str | None:
        ...
        if not await cast(Any, self._circuit_breaker).can_execute():
            # FAIL-OPEN: Bypass cache when circuit is open
            await cast(Any, self._circuit_breaker).record_rejection()
            self._bypass_count += 1
            logger.debug(
        ...
        try:
            result = await cast(Any, self._redis).get(key)
            await cast(Any, self._circuit_breaker).record_success()
            return str(result) if result is not None else None
        except _REDIS_OPERATION_ERRORS as e:
            await cast(Any, self._circuit_breaker).record_failure(e, type(e).__name__)
    ...
    async def set(self, key: str, value: str, ex: int | None = None) -> bool:
        ...
        try:
            if ex:
                await cast(Any, self._redis).setex(key, ex, value)
            else:
                await cast(Any, self._redis).set(key, value)
            await cast(Any, self._circuit_breaker).record_success()
            return True
        except _REDIS_OPERATION_ERRORS as e:
            await cast(Any, self._circuit_breaker).record_failure(e, type(e).__name__)
    ...
    async def delete(self, *keys: str) -> int:
        ...
        try:
            result = await cast(Any, self._redis).delete(*keys)
            await cast(Any, self._circuit_breaker).record_success()
            return int(result)
        except _REDIS_OPERATION_ERRORS as e:
            await cast(Any, self._circuit_breaker).record_failure(e, type(e).__name__)
    ...
    async def batch_get(self, keys: list[str]) -> list[str | None]:
        ...
        try:
            pipe = cast(Any, self._redis).pipeline()
            for key in keys:
                pipe.get(key)
            results = await pipe.execute()
            await cast(Any, self._circuit_breaker).record_success()
            return cast(list[str | None], list(results))
        except _REDIS_OPERATION_ERRORS as e:
            await cast(Any, self._circuit_breaker).record_failure(e, type(e).__name__)
            self._degraded_operations += 1
    ...
    async def health_check(self) -> JSONDict:
        ...
        if self._circuit_breaker:
            health["circuit_state"] = cast(Any, self._circuit_breaker).state.value
            health["circuit_metrics"] = cast(Any, self._circuit_breaker).metrics.__dict__
    ...
    def get_circuit_status(self) -> JSONDict:
        ...
        return cast(JSONDict, cast(Any, self._circuit_breaker).get_status())
