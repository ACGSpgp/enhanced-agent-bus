...
class _TenantRepositoryProtocol(Protocol):
...
async def get_system_tenant(repository: _TenantRepositoryProtocol) -> Tenant | None:
    ...
    """
    return await repository.get_tenant(SYSTEM_TENANT_ID)


async def ensure_system_tenant(repository: _TenantRepositoryProtocol) -> Tenant:
    ...
    return await repository.create_tenant(
        ...
        quota=cast(dict[str, object], defaults.quota),
        metadata=cast(dict[str, object], defaults.metadata),
        tenant_id=SYSTEM_TENANT_ID,
    )
