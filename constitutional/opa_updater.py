...
class OPAPolicyUpdater:
    ...
    async def connect(self) -> None:
        ...
        if OPAClient is not None:
            try:
                ...
                logger.warning("Failed to initialize OPA client: %s", str(e))
        ...
        if AuditClient is not None:
            try:
                ...
                logger.warning("Failed to initialize audit client: %s", str(e))
    ...
    async def update_policy(self, request: PolicyUpdateRequest) -> PolicyUpdateResult:
        ...
        try:
            ...
            logger.error("Update %s: Policy update failed with exception: %s", update_id, str(e))
            result.status = PolicyUpdateStatus.FAILED
    ...
    async def _validate_policy(self, request: PolicyUpdateRequest) -> PolicyValidationResult:
        ...
        try:
            ...
            logger.error("HTTP error during policy validation: %s", str(e))
            validation.errors.append(f"HTTP error: {e!s}")
            return validation
        except (RuntimeError, ValueError, TypeError) as e:
            logger.error("Unexpected error during policy validation: %s", str(e))
    ...
    async def _backup_current_policy(self, policy_id: str) -> str | None:
        ...
        try:
            ...
            logger.error("Error backing up policy %s: %s", policy_id, str(e))
            return None
    ...
    async def _upload_policy_to_opa(self, request: PolicyUpdateRequest) -> bool:
        ...
        try:
            ...
            logger.error("Error uploading policy %s: %s", request.policy_id, str(e))
            return False
    ...
    async def _health_check_opa(self) -> bool:
        ...
        if self._opa_client:
            try:
                health_result = await cast(Any, self._opa_client).health_check()
                ...
                logger.error("OPA health check failed: %s", str(e))
                return False
    ...
    async def _invalidate_cache(self, policy_id: str) -> bool:
        ...
        try:
            ...
            await cast(Any, self._opa_client).clear_cache(policy_path=policy_path)
            logger.info("Cache invalidated for policy %s", policy_id)
            return True
        except (RuntimeError, ValueError, TypeError) as e:
            logger.error("Failed to invalidate cache for policy %s: %s", policy_id, str(e))
            return False
    ...
    async def _rollback_policy(
        ...
        try:
            ...
            logger.error("Error rolling back policy %s: %s", policy_id, str(e))
            return False
    ...
    async def _emit_policy_event(self, result: PolicyUpdateResult, event_type: str) -> None:
        ...
        if self._audit_client:
            try:
                ...
                logger.warning("Failed to submit policy event to audit service: %s", str(e))
