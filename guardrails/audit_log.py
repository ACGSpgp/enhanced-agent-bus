...
class AuditSIEMProviderProtocol(Protocol):
    """Subset of SIEM provider API used by the audit log."""

    async def send_event(self, event: JSONDict) -> bool:
        """Ship an audit event to the provider."""
        ...
...
class AuditLog(GuardrailComponent):
    ...
    def __init__(self, config: AuditLogConfig | None = None):
        ...
        self._siem_providers: list[AuditSIEMProviderProtocol] = []
        self._siem_metrics = {
            "events_sent": 0,
            "events_failed": 0,
        ...
        try:
            ...
            for provider_config in self.config.siem_providers:
                ...
                try:
                    ...
                    provider_type_value = provider_config.get("provider_type", "")
                    endpoint_url_value = provider_config.get("endpoint_url", "")
                    auth_token_value = provider_config.get("auth_token", "")
                    provider_type_str = (
                        provider_type_value if isinstance(provider_type_value, str) else ""
                    )
                    endpoint_url = endpoint_url_value if isinstance(endpoint_url_value, str) else ""
                    auth_token = auth_token_value if isinstance(auth_token_value, str) else ""
                    ...
                    config = SIEMProviderConfig(
                        ...
                        index=str(provider_config.get("index", "acgs2_audit")),
                        source_type=str(provider_config.get("source_type", "acgs2:audit")),
                        verify_ssl=bool(provider_config.get("verify_ssl", True)),
                        timeout_seconds=float(
                            provider_config.get("timeout_seconds", self.config.siem_timeout_seconds)
                        ),
        ...
        for i, provider in enumerate(self._siem_providers):
            ...
            try:
                if hasattr(provider, "health_check"):
                    health = await cast(Any, provider).health_check()
