...
class AsyncMemoryQueue:
    ...
    def __init__(
        ...
        self._queue: asyncio.Queue[MemorySnapshot] = asyncio.Queue(maxsize=config.queue_size)
        self._running = False
...
def profile_memory(
    operation: str,
) -> Callable[[Callable[P, asyncio.Future[R] | Any]], Callable[P, asyncio.Future[R] | Any]]:
    """
    ...
    def decorator(
        func: Callable[P, asyncio.Future[R] | Any],
    ) -> Callable[P, asyncio.Future[R] | Any]:
        ...
        async def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            ...
            trace_id_value = kwargs.get("trace_id")
            fallback_trace_id = getattr(args[0], "trace_id", None) if args else None
            trace_id = trace_id_value if isinstance(trace_id_value, str) else None
            if trace_id is None and isinstance(fallback_trace_id, str):
