...
class _SettingsProtocol(Protocol):
    redis: _RedisSettingsProtocol


class _RedisMetricsClientProtocol(Protocol):
    async def close(self) -> object: ...
    async def zadd(self, key: str, mapping: dict[str, float]) -> object: ...
    async def hincrby(self, key: str, field: str, amount: int) -> object: ...
    async def zremrangebyscore(self, key: str, minimum: str | float, maximum: float) -> object: ...
...
class _FallbackSettings:
    redis: _RedisSettingsProtocol = _FallbackRedisSettings()


try:
    from enhanced_agent_bus._compat.config import settings as _imported_settings
except ImportError:
    _settings_candidate: _SettingsProtocol = _FallbackSettings()
else:
    _settings_candidate = cast(_SettingsProtocol, _imported_settings)

settings = _settings_candidate
...
class MetricsComparison(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        """Compute deltas and degradation flags after validation."""
        del __context
...
class GovernanceMetricsCollector:
    ...
    def __init__(
        ...
        self.redis_client: _RedisMetricsClientProtocol | None = None
    ...
    def _coerce_int(value: object, default: int = 0) -> int:
        """Best-effort conversion for Redis scalar values."""
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self.redis_client = cast(
                _RedisMetricsClientProtocol,
                await redis_module.from_url(
                    self.redis_url,
                    encoding="utf-8",
    ...
    async def get_baseline_snapshot(
        ...
        try:
            ...
            if data:
                baseline: GovernanceMetricsSnapshot = GovernanceMetricsSnapshot.model_validate_json(
                    data
                )
                return baseline
            return None
    ...
    async def _get_latencies(self, start: float, end: float) -> list[float]:
        ...
        try:
            ...
            latency_strs = await redis_client.zrangebyscore(
    ...
    async def _get_request_counters(self) -> dict[str, int]:
        ...
        try:
            counters_raw = await redis_client.hgetall("governance:metrics:requests")
            counters = counters_raw if isinstance(counters_raw, Mapping) else {}
            return {
                "total": self._coerce_int(counters.get("total", 0)),
                "approved": self._coerce_int(counters.get("approved", 0)),
                "denied": self._coerce_int(counters.get("denied", 0)),
                "escalated": self._coerce_int(counters.get("escalated", 0)),
            }
        except (RuntimeError, ValueError, TypeError) as e:
    ...
    async def _get_deliberation_metrics(self) -> JSONDict:
        ...
        try:
            counters_raw = await redis_client.hgetall("governance:metrics:deliberations")
            counters = counters_raw if isinstance(counters_raw, Mapping) else {}
            total = self._coerce_int(counters.get("total", 0))
            success = self._coerce_int(counters.get("success", 0))
            failed = self._coerce_int(counters.get("failed", 0))
    ...
    async def _store_snapshot(self, snapshot: GovernanceMetricsSnapshot) -> None:
        ...
        try:
            ...
            await redis_client.zadd("governance:metrics:snapshots:index", {key: timestamp})
