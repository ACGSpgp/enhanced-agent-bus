...
class MACIConfigLoader:
    ...
    def load_from_env(self) -> MACIConfig:
        ...
        if strict_env is not None:
            ...
            strict = bool(getattr(cast(Any, global_settings).maci, "strict_mode", True))
        else:
            strict = True
        def_role_str: str | None
        if def_role_env is not None:
            def_role_str = def_role_env
        elif global_settings is not None:
            default_role = getattr(cast(Any, global_settings).maci, "default_role", None)
            def_role_str = default_role if isinstance(default_role, str) else None
        else:
            def_role_str = None
