...
def _as_object_dict(value: object) -> dict[str, object]:
    return cast(dict[str, object], _deserialize_json_value(value))


def _as_object_dict_or_none(value: object) -> dict[str, object] | None:
    return cast(dict[str, object] | None, _deserialize_json_value(value))


def _as_any_dict(value: object) -> dict[str, Any]:
    return cast(dict[str, Any], _deserialize_json_value(value))


def _as_any_dict_or_none(value: object) -> dict[str, Any] | None:
    return cast(dict[str, Any] | None, _deserialize_json_value(value))


def _as_any_dict_list(value: object) -> list[dict[str, Any]]:
    return cast(list[dict[str, Any]], _deserialize_json_value(value))
...
class PostgresWorkflowRepository(WorkflowRepository):
    ...
    def _row_to_workflow(self, row: asyncpg.Record) -> WorkflowInstance:
        return WorkflowInstance(
            ...
            input=_as_object_dict_or_none(row["input"]),
            output=_as_object_dict_or_none(row["output"]),
            error=row["error"],
    ...
    def _row_to_event(self, row: asyncpg.Record) -> WorkflowEvent:
        return WorkflowEvent(
            ...
            event_data=_as_object_dict(row["event_data"]),
    ...
    def _row_to_checkpoint(self, row: asyncpg.Record) -> CheckpointData:
        return CheckpointData(
            ...
            state=_as_object_dict(row["state"]),
            created_at=row["created_at"],
        )
    ...
    def _row_to_decision_event(self, row: asyncpg.Record) -> DecisionEvent:
        return DecisionEvent(
            ...
            request_context=_as_any_dict(row["request_context"]),
            decision_payload=_as_any_dict(row["decision_payload"]),
    ...
    def _row_to_candidate_artifact(self, row: asyncpg.Record) -> CandidateArtifact:
        return CandidateArtifact(
            ...
            candidate_spec=_as_any_dict(row["candidate_spec"]),
    ...
    def _row_to_evaluation_run(self, row: asyncpg.Record) -> EvaluationRun:
        return EvaluationRun(
            ...
            status=row["status"],
            summary_metrics=_as_any_dict(row["summary_metrics"]),
            started_at=row["started_at"],
    ...
    def _row_to_evidence_bundle(self, row: asyncpg.Record) -> EvidenceBundle:
        return EvidenceBundle(
            ...
            validator_records=_as_any_dict_list(row["validator_records"]),
            rollback_plan=_as_any_dict(row["rollback_plan"]),
            artifact_manifest_uri=row["artifact_manifest_uri"],
            created_at=row["created_at"],
        )
