...
def get_dependency_registry() -> DependencyRegistry:
    """Return a process-local dependency registry singleton.

    The shared source module exposes the ``DependencyRegistry`` class but not a
    convenience accessor. This compatibility shim preserves the legacy
    ``enhanced_agent_bus._compat.utilities`` API expected by package imports.
    """
