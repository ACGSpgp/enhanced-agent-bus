...
class AuditClient:
    ...
    def __init__(
        ...
        self._recent_results: deque[AuditBatchResult] = deque(maxlen=100)
            ...
            for attempt in range(self.config.max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        audit_hash = (
                            result_payload.get("entry_hash")
                            if isinstance(result_payload, dict)
                            else None
                        )
                        self._stats["successful"] += 1
                        logger.info(f"Audit record successful: {audit_hash}")
                        return audit_hash if isinstance(audit_hash, str) else None
                    else:
                        logger.error(
    ...
    async def get_stats(self) -> JSONDict:
        ...
        try:
            ...
            result_payload = response.json()
            remote_stats = result_payload if isinstance(result_payload, dict) else {}
        except (
            httpx.RequestError,
            RuntimeError,
            ValueError,
