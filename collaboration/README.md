...
JWT posture for the collaboration API:

- bearer token validation uses `COLLABORATION_SECRET_KEY`
- `COLLABORATION_JWT_ALGORITHM` is intentionally `HS256`-only
- global `JWT_ALGORITHM` does not configure collaboration auth

If collaboration ever needs asymmetric JWTs, public-key loading must be added
before widening the allowed algorithm set.
