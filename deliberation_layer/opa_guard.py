...
from typing import TYPE_CHECKING, Any, Callable, ParamSpec, TypeAlias, TypeVar, cast
...
try:
    _models_module = importlib.import_module("enhanced_agent_bus.models")
    _opa_client_module = importlib.import_module("enhanced_agent_bus.opa_client")
    _validators_module = importlib.import_module("enhanced_agent_bus.validators")
except ImportError:
    _models_module = importlib.import_module("enhanced_agent_bus.models")
    _opa_client_module = importlib.import_module("opa_client")
    _validators_module = importlib.import_module("enhanced_agent_bus.validators")

import sys as _sys

try:
    from . import opa_guard_models as _opa_guard_models_module  # type: ignore[assignment]
except ImportError:
    try:
        _opa_guard_models_module = importlib.import_module(
            "enhanced_agent_bus.deliberation_layer.opa_guard_models"
        )
    except ImportError:
        _opa_guard_models_module = importlib.import_module("opa_guard_models")

# Ensure consistent module identity: register under all canonical import paths so
# any consumer (regardless of how they spell the import) gets the same class objects.
_sys.modules.setdefault("deliberation_layer.opa_guard_models", _opa_guard_models_module)
_sys.modules.setdefault(
    "enhanced_agent_bus.deliberation_layer.opa_guard_models", _opa_guard_models_module
)
_sys.modules.setdefault(
    "packages.enhanced_agent_bus.deliberation_layer.opa_guard_models", _opa_guard_models_module
)

CONSTITUTIONAL_HASH = _models_module.CONSTITUTIONAL_HASH
get_opa_client = _opa_client_module.get_opa_client
GUARD_CONSTITUTIONAL_HASH = _opa_guard_models_module.GUARD_CONSTITUTIONAL_HASH
_CriticReviewRuntime = _opa_guard_models_module.CriticReview
_GuardDecisionRuntime = _opa_guard_models_module.GuardDecision
_GuardResultRuntime = _opa_guard_models_module.GuardResult
_ReviewResultRuntime = _opa_guard_models_module.ReviewResult
_ReviewStatusRuntime = _opa_guard_models_module.ReviewStatus
_SignatureRuntime = _opa_guard_models_module.Signature
_SignatureResultRuntime = _opa_guard_models_module.SignatureResult
_SignatureStatusRuntime = _opa_guard_models_module.SignatureStatus
ValidationResult = _validators_module.ValidationResult
# Runtime aliases so consumers can import these names directly
CriticReview = _CriticReviewRuntime
GuardDecision = _GuardDecisionRuntime
GuardResult = _GuardResultRuntime
ReviewResult = _ReviewResultRuntime
ReviewStatus = _ReviewStatusRuntime
Signature = _SignatureRuntime
SignatureResult = _SignatureResultRuntime
SignatureStatus = _SignatureStatusRuntime
...
_GuardParams = ParamSpec("_GuardParams")
_GuardReturn = TypeVar("_GuardReturn")


def _typed_fail_closed(
    fallback: Callable[..., _GuardReturn],
    *,
    exceptions: tuple[type[BaseException], ...],
) -> Callable[
    [Callable[_GuardParams, _GuardReturn]],
    Callable[_GuardParams, _GuardReturn],
]:
    return cast(
        Callable[
        ...
        fail_closed(fallback, exceptions=exceptions),
    )
...
class OPAGuard:
    ...
    async def verify_action(
        ...
        try:
            ...
            if not constitutional_valid:
                ...
                result.decision = _GuardDecisionRuntime.DENY
            ...
            if not policy_result.get("allowed", False):
                ...
                result.decision = _GuardDecisionRuntime.REQUIRE_REVIEW
                result.is_allowed = False
                ...
                result.decision = _GuardDecisionRuntime.REQUIRE_SIGNATURES
                result.is_allowed = False
                result.requires_signatures = True
                ...
                result.decision = _GuardDecisionRuntime.ALLOW
    ...
    async def collect_signatures(
        ...
        signature_result = cast(
            SignatureResult,
            _SignatureResultRuntime(
        ...
        cast(Any, signature_result)._completion_event = sig_event

        try:
            await asyncio.wait_for(sig_event.wait(), timeout=timeout)
        except TimeoutError:
            signature_result.status = _SignatureStatusRuntime.EXPIRED
        ...
        elif signature_result.status == _SignatureStatusRuntime.REJECTED:
    ...
    async def submit_signature(
        ...
        if success:
            ...
            if event and (
                signature_result.is_complete
                or signature_result.status == _SignatureStatusRuntime.REJECTED
            ):
                event.set()
    ...
    async def submit_for_review(
        ...
        decision_id = get_str(decision, "id", "") or str(uuid.uuid4())

        review_result = cast(
            ReviewResult,
            _ReviewResultRuntime(
                decision_id=decision_id,
                ...
                if callable(callback):
                    try:
                        ...
                        def _log_callback_result(
                            task: asyncio.Task[object], cid: str = _cid
                        ) -> None:
                            task_exception = task.exception()
                            if task_exception is not None:
                                logger.warning(
                                    "Critic callback failed",
                                    extra={"critic_id": cid, "error": str(task_exception)},
                                )

                        _t.add_done_callback(_log_callback_result)
                    except _OPA_GUARD_OPERATION_ERRORS as e:
        ...
        for _ in range(max_poll_iterations):
            ...
            if elapsed >= timeout:
                if not review_result.consensus_reached:
                    review_result.status = _ReviewStatusRuntime.ESCALATED
    ...
    async def log_decision(self, decision: JSONDict, result: JSONDict) -> None:
        """
    ...
    async def check_constitutional_compliance(self, action: JSONDict) -> bool:
        ...
        """
        return bool(await self._check_constitutional_compliance_impl(action))
    ...
    async def _check_constitutional_compliance_impl(self, action: JSONDict) -> bool:
        ...
        if is_json_dict(result):
            return bool(get_bool(result, "allowed", default_value))
    ...
    async def evaluate(
        ...
        """
        return cast(JSONDict, await self._evaluate_impl(message_data, policy_path))
    ...
    def get_audit_log(
        ...
        if agent_id:
            filtered_logs: list[JSONDict] = []
            for log in logs:
                decision = log.get("decision")
                if isinstance(decision, dict) and decision.get("agent_id") == agent_id:
...
async def initialize_opa_guard(**kwargs: object) -> OPAGuard:
    """
    ...
    """
    global _opa_guard
    _opa_guard = OPAGuard(**cast(dict[str, Any], kwargs))
