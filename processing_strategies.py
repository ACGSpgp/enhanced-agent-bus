...
if TYPE_CHECKING:
    ...
    try:
        from .maci_imports import MACIError as _RuntimeMACIError
    except (ImportError, ValueError):

        class _RuntimeMACIError(RuntimeError):
            """Fallback MACI error type for strategy error tuples."""

            pass

    MACIError = _RuntimeMACIError
...
class RustProcessingStrategy(HandlerExecutorMixin):
    ...
    def __init__(
        self,
        rust_processor: object | None = None,
        rust_bus: object | None = None,
        validation_strategy: RustValidationStrategy | None = None,
        metrics_enabled: bool = False,
    ) -> None:
        ...
        self._rb = rust_bus
        self._validation_strategy = validation_strategy or RustValidationStrategy(
            cast(Any, rust_processor)
        )
        self._metrics_enabled: bool = metrics_enabled
    ...
    def _to_rust(self, msg: AgentMessage) -> object:
        if self._rb is None:
            raise RuntimeError("Rust bus bindings are not initialized")
        rb = cast(Any, self._rb)
        r = rb.AgentMessage()
        ...
        if hasattr(rb, "Priority"):
            ...
            r.priority = getattr(rb.MessagePriority, msg.priority.name.capitalize(), None)
        if hasattr(rb, "MessageStatus"):
            r.status = getattr(rb.MessageStatus, msg.status.name.capitalize(), None)
        return r
...
class DynamicPolicyProcessingStrategy(PythonProcessingStrategy):
    ...
    def __init__(
        ...
        validation_strategy: DynamicPolicyValidationStrategy | None = None,
        metrics_enabled: bool = False,
    ) -> None:
...
class OPAProcessingStrategy(PythonProcessingStrategy):
    ...
    def __init__(
        ...
        validation_strategy: OPAValidationStrategy | None = None,
        metrics_enabled: bool = False,
    ) -> None:
        ...
        """
        super().__init__(
            validation_strategy or OPAValidationStrategy(cast(Any, opa_client)),
            metrics_enabled,
        )
...
class MACIProcessingStrategy:
        ...
        if callable(validate_action):
            try:
                if available("maci_strategy"):
                    validator_cls = cast(
                        Any,
                        import_module(require("maci_strategy")).MACIValidationStrategy,
                    )
                    return cast(object, validator_cls(maci_enforcer))
            except ValueError:
                return maci_enforcer
    ...
    def _deny_maci_processing(
        ...
        *,
        ...
        del msg, handlers
        return self._handle_maci_validation_error(error)
    ...
    async def _process_with_maci_validation(
        ...
        try:
            ...
            return self._deny_maci_processing(msg, handlers, error=error)
