...
_rust_opt: Any = None
try:
    import enhanced_agent_bus_rust.optimization as _rust_opt_mod

    _rust_opt = _rust_opt_mod
    RUST_AVAILABLE = True
except ImportError:
    RUST_AVAILABLE = False

_sinkhorn_projection: Any
try:
    from ..governance.stability.mhc import sinkhorn_projection as _sinkhorn_projection
except (ImportError, ValueError):
    _sinkhorn_projection = cast(Any, None)
...
async def _await_maybe(value: object) -> object:
    """Await values that may be sync or async depending on runtime/stubs."""
    if hasattr(value, "__await__"):
        return await cast(Any, value)
    return value


class VotePubSubProtocol(Protocol):
    """Subset of redis pubsub API used by the vote collector."""

    async def subscribe(self, channel: str) -> object: ...
    async def unsubscribe(self, *channels: str) -> object: ...
    async def close(self) -> object: ...
    async def get_message(self, ignore_subscribe_messages: bool = True) -> JSONDict | None: ...


class VoteRedisClientProtocol(Protocol):
    """Subset of redis client API used by the vote collector."""

    def ping(self) -> object: ...
    async def close(self) -> object: ...
    def pubsub(self) -> VotePubSubProtocol: ...
    ...
    async def hdel(self, key: str, field: str) -> object: ...
    ...
    def _stabilize_weights(self) -> dict[str, float]:
        ...
        if RUST_AVAILABLE:
            try:
                ...
                weights_matrix = _np.array([weights_list], dtype=_np.float32)

                # Stabilize using Sinkhorn-Knopp algorithm in Rust
                stabilized_matrix = _rust_opt.sinkhorn_knopp_stabilize(
        ...
        if _sinkhorn_projection is None or not TORCH_AVAILABLE:
            return self.agent_weights
...
class EventDrivenVoteCollector:
    ...
    def __init__(
        ...
        # Redis connections
        self.redis_client: VoteRedisClientProtocol | None = None
        self.pubsub: VotePubSubProtocol | None = None
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            await _await_maybe(self.redis_client.ping())
            logger.info(f"Vote collector connected to Redis at {redis_url}")
    ...
    async def wait_for_consensus(
        ...
        try:
            ...
            if completion_event is None:
                return {"error": "Session completion event missing", "session_id": session_id}
    ...
    async def get_current_votes(self, message_id: str) -> list[JSONDict]:
        ...
        if self.redis_client:
            try:
                ...
                votes: list[JSONDict] = []
                for raw_vote in votes_raw.values():
        ...
        in_memory_votes = self._in_memory_votes.get(message_id, [])
        return [v.to_dict() for v in in_memory_votes]
