...
try:
    _models = importlib.import_module("enhanced_agent_bus.models")
except (ImportError, ValueError):
    _models = importlib.import_module("models")

_AgentMessageRuntime = _models.AgentMessage
_MessageStatusRuntime = _models.MessageStatus
get_enum_value = _models.get_enum_value
...
_all_queue_instances: list["DeliberationQueue"] = []
...
class DeliberationQueue:
    ...
    def __init__(
        ...
        self._bg_tasks: set[asyncio.Task[None]] = set()  # Prevents GC of fire-and-forget tasks
    ...
    def _load_tasks(self) -> None:
        ...
        try:
            ...
            for tid, tdata in data.items():
                ...
                msg = _AgentMessageRuntime.from_dict(tdata["message"])
    ...
    async def enqueue(self, *args: object, **kwargs: object) -> str:
        ...
        return cast(str, await cast(Any, self.enqueue_for_deliberation)(*args, **kwargs))
    ...
    async def resolve_task(self, task_id: str, approved: bool) -> None:
        ...
        if approved:
            task.message.status = _MessageStatusRuntime.PENDING  # Ready for re-delivery
        else:
            task.message.status = _MessageStatusRuntime.FAILED


_deliberation_queue: DeliberationQueue | None = None
