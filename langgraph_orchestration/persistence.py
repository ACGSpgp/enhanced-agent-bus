...
class _RedisPipelineProtocol(Protocol):
    def get(self, key: str) -> object: ...
    async def execute(self) -> list[object]: ...


class _RedisClientProtocol(Protocol):
    async def setex(self, key: str, ttl: int, data: str) -> object: ...
    async def get(self, key: str) -> str | None: ...
    def scan_iter(self, pattern: str) -> Any: ...
    def pipeline(self) -> _RedisPipelineProtocol: ...
    async def zadd(self, key: str, mapping: dict[str, float]) -> object: ...
    async def expire(self, key: str, ttl: int) -> object: ...
    async def zrange(self, key: str, start: int, end: int) -> list[object]: ...
    async def delete(self, key: str) -> int: ...
    async def zrem(self, key: str, member: str) -> object: ...
...
class RedisStatePersistence(StatePersistence):
    ...
    def __init__(
        ...
        self.constitutional_hash = constitutional_hash
        self._redis: _RedisClientProtocol | None = None

    async def _get_redis(self) -> _RedisClientProtocol:
        """Get or create Redis connection."""
        if self._redis is None:
            try:
                import redis.asyncio as aioredis

                self._redis = cast(_RedisClientProtocol, await aioredis.from_url(self.redis_url))
            except ImportError:
            ...
            for raw_data in results:
                if isinstance(raw_data, str):
                    ...
                    if isinstance(parsed, dict):
                        created = datetime.fromisoformat(str(parsed["created_at"]))
                        if latest_time is None or created > latest_time:
                            latest = parsed
                            latest_time = created

            if latest:
                state_payload = latest.get("state")
                if isinstance(state_payload, dict):
                    return GraphState(**state_payload)
...
def create_state_persistence(
    ...
    **kwargs: object,
    ...
    Constitutional Hash: 608508a9bd224290
    """
    constitutional_hash_raw = kwargs.pop("constitutional_hash", CONSTITUTIONAL_HASH)
    constitutional_hash = (
        constitutional_hash_raw if isinstance(constitutional_hash_raw, str) else CONSTITUTIONAL_HASH
    )
    ...
    if backend == "memory":
        ...
        return RedisStatePersistence(
            ...
            key_prefix=str(kwargs.get("key_prefix", "acgs2:langgraph:")),
            ttl_seconds=int(str(kwargs.get("ttl_seconds", 86400))),
