...
_HuggingFaceAdapter: Any
_LLMMessage: Any
try:
    from enhanced_agent_bus.llm_adapters import (
        HuggingFaceAdapter as _HuggingFaceAdapterImport,
    )
    from enhanced_agent_bus.llm_adapters import (
        LLMMessage as _LLMMessageImport,
    )

    _HUGGINGFACE_ADAPTER_AVAILABLE = True
    _HuggingFaceAdapter = _HuggingFaceAdapterImport
    _LLMMessage = _LLMMessageImport
except ImportError:
    _HuggingFaceAdapter = None
    _LLMMessage = None

HuggingFaceAdapter = _HuggingFaceAdapter
LLMMessage = _LLMMessage


class _AdapterResponseProtocol(Protocol):
    content: object


class _HuggingFaceAdapterProtocol(Protocol):
    async def acomplete(self, *, messages: list[object]) -> object: ...
...
class LocoOperatorGovernanceClient:
        ...
        if _HUGGINGFACE_ADAPTER_AVAILABLE and HuggingFaceAdapter is not None:
            try:
                ...
                logger.warning(
                    ...
                    f"falling back to unavailable mode: {exc}"
                )
    ...
    async def score_governance_action(
        ...
        try:
            ...
            score = float(raw_score if isinstance(raw_score, (int, float, str)) else 0.5)
    ...
    async def evaluate_policy_fragment(
        ...
        try:
            ...
            confidence = float(
                raw_confidence if isinstance(raw_confidence, (int, float, str)) else 0.5
            )
    ...
    async def health_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            if hasattr(adapter, "health_check"):
                await cast(_HuggingFaceAdapterProtocol, adapter).health_check()
            latency_ms = (time.monotonic() - t_start) * 1000.0
            return HealthCheckResult(
                is_healthy=True,
                model_id=self._config.model,
            ...
            latency_ms = (time.monotonic() - t_start) * 1000.0
            logger.warning(f"LocoOperator health_check failed: {exc}")
            return HealthCheckResult(
                is_healthy=False,
                model_id=self._config.model,
                latency_ms=latency_ms,
    ...
    async def _call_adapter(self, system: str, user: str) -> str:
        ...
        adapter = cast(_HuggingFaceAdapterProtocol, self._adapter)
        llm_message_factory = cast(Any, LLMMessage)
        messages = [
            llm_message_factory(role="system", content=system),
            llm_message_factory(role="user", content=user),
        ]

        try:
            response = await adapter.acomplete(messages=messages)
