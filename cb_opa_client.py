...
class CircuitBreakerOPAClient:
    ...
    async def close(self) -> None:
        ...
        if self._http_client:
            try:
                ...
                close_coro = cast(Any, self._http_client).aclose()
    ...
    def _require_circuit_breaker(self) -> ServiceCircuitBreaker:
        if self._circuit_breaker is None:
            raise RuntimeError("OPA circuit breaker not initialized")
        return self._circuit_breaker

    def _require_http_client(self) -> Any:
        if self._http_client is None:
            raise RuntimeError("OPA HTTP client not initialized")
    ...
    async def evaluate_policy(
        ...
        breaker = self._require_circuit_breaker()
        if not await breaker.can_execute():
            # FAIL-CLOSED: Deny all requests when circuit is open
            await breaker.record_rejection()
            logger.warning(
    ...
    async def _evaluate_policy_with_fail_closed(
        ...
        await self._require_circuit_breaker().record_success()
    ...
    async def _handle_evaluate_policy_error(self, error: BaseException) -> JSONDict:
        ...
        breaker = self._require_circuit_breaker()
        breaker_error = error if isinstance(error, Exception) else RuntimeError(str(error))
        await breaker.record_failure(breaker_error, error_type)
        logger.error(
    ...
    async def _evaluate_http(self, input_data: JSONDict, policy_path: str) -> JSONDict:
        ...
        response = await self._require_http_client().post(url, json={"input": input_data})
    ...
    async def _populate_health(self, health: JSONDict) -> JSONDict:
        response = await self._require_http_client().get(f"{self.opa_url}/health", timeout=2.0)
        response.raise_for_status()
        health["healthy"] = True
