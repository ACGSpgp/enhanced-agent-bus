...
if TYPE_CHECKING:
    from enhanced_agent_bus.deliberation_layer.redis_election_store import RedisElectionStore
else:
    RedisElectionStore = Any

try:
    from enhanced_agent_bus.deliberation_layer import redis_election_store as _redis_election_store
except ImportError:
    _get_election_store: Callable[[], Any] | None = None
else:
...
class _SettingsProtocol(Protocol):
    voting: _VotingSettingsProtocol


class _VoteEventBusProtocol(Protocol):
    async def publish_vote_event(
        self,
        tenant_id: object,
        vote_event_dict: dict[str, object],
    ) -> bool: ...


try:
    from enhanced_agent_bus._compat.config import settings as _loaded_settings
except ImportError:
    settings: _SettingsProtocol | None = None
else:
    settings = cast(_SettingsProtocol, _loaded_settings)
...
class ElectionsDict(Mapping[str, ElectionProxy]):
    """Mapping wrapper that returns ElectionProxy objects for backward compat."""

    def __init__(self, raw: dict[str, JSONDict]) -> None:
        self._raw = raw

    def __getitem__(self, key: str) -> ElectionProxy:
        raw = self._raw[key]
        return ElectionProxy(raw)
...
class VotingService:
    ...
    async def _ensure_store_initialized(self) -> bool:
        ...
        if self.election_store is None:
            if _get_election_store is None:
                logger.warning("RedisElectionStore not available, using in-memory fallback")
                return False
            try:
                self.election_store = cast("RedisElectionStore", await _get_election_store())
    ...
    async def create_election(
        ...
        if timeout is None:
            if active_settings is None:
                ...
                timeout = active_settings.voting.default_timeout_seconds
        ...
        if await self._ensure_store_initialized() and self.election_store is not None:
    ...
    async def _publish_vote_event(
        ...
        if not isinstance(participant_weights, dict):
            participant_weights = {}
        vote_event_dict = {
            "election_id": election_id,
            "agent_id": vote.agent_id,
            "decision": vote.decision,
            "weight": participant_weights.get(vote.agent_id, 1.0),
    ...
    async def _finalize_election_result(
        ...
        if await self._ensure_store_initialized() and self.election_store:
            active_settings = settings
            ttl = (
                active_settings.voting.default_timeout_seconds
    ...
    async def _get_closed_election_result(
        ...
        await self._check_resolution(election_id)
        refreshed_election = await self._get_election_data(election_id)
        return cast(str | None, refreshed_election.get("result")) if refreshed_election else None
