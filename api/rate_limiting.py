...
class _RedisRateLimitClientProtocol(Protocol):
    async def incrby(self, key: str, amount: int) -> int: ...

    async def expire(self, key: str, ttl_seconds: int) -> object: ...


_DecoratedCallable = TypeVar("_DecoratedCallable", bound=Callable[..., object])
...
_redis_client: _RedisRateLimitClientProtocol | None = None
try:
    import redis.asyncio as aioredis

    _redis_url = os.environ.get("REDIS_URL", "redis://localhost:6379")
    _redis_client = cast(
        _RedisRateLimitClientProtocol,
...
if TYPE_CHECKING:
    ...
    try:
        ...
        from ..fallback_stubs import (
            get_remote_address as _fallback_get_remote_address,
        )

        def get_remote_address(request: Request) -> str:
            return _fallback_get_remote_address()

        logger.warning(
            "slowapi not installed; API rate limiting is disabled and using fallback stubs"
        )
...
async def _check_rate_limit_redis(client_id: str, rate_limit_cost: int, batch_size: int) -> None:
    ...
    if redis_client is None:
        raise RuntimeError("Redis client unavailable for rate limiting")

    new_count = await redis_client.incrby(window_key, rate_limit_cost)

    # Set expiry on first request in window (TTL = window duration + 60s buffer)
    if new_count == rate_limit_cost:
        await redis_client.expire(window_key, RATE_LIMIT_WINDOW_DURATION_MINUTES * 60 + 60)
...
storage_uri = os.environ.get("RATE_LIMIT_STORAGE_URI") or os.environ.get("REDIS_URL")
limiter: _LimiterProtocol
if storage_uri:
    limiter = cast(
        _LimiterProtocol,
        Limiter(key_func=get_remote_address, storage_uri=storage_uri),
    )
else:
    limiter = cast(_LimiterProtocol, Limiter(key_func=get_remote_address))
