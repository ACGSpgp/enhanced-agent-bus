...
# Global state
agent_bus: MessageProcessor | dict[str, Any] | None = None
...
def _initialize_batch_processor_state(
    ...
    try:
        processor = BatchMessageProcessor(
            message_processor=cast(Any, message_processor),
...
async def _stop_cache_warmer_if_running() -> None:
    ...
    try:
        from .. import _ext_cache_warming as cache_warming

        if (
            cache_warming.CACHE_WARMING_AVAILABLE
            and (get_cache_warmer := getattr(cache_warming, "get_cache_warmer", None)) is not None
...
def _bind_runtime_state(app: FastAPI, *, bus: MessageProcessor | dict[str, Any]) -> None:
...
@asynccontextmanager
async def _lifespan_context(app: FastAPI) -> Any:
...
def _register_exception_handlers(app: FastAPI) -> None:
    ...
    handlers: list[tuple[type[Exception], Callable[[Any, Exception], Awaitable[Any] | Any]]] = [
    ...
    for exc_class, handler in handlers:
        app.add_exception_handler(exc_class, cast(Any, handler))
