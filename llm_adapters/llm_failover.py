...
class ProviderWarmupManager:
    ...
    def __init__(self) -> None:
        ...
        self._warmup_tasks: dict[str, asyncio.Task[Any]] = {}
        self._lock = asyncio.Lock()
...
class RequestHedgingManager:
    ...
    def _process_completed_tasks(
        self, done: set[asyncio.Task[Any]], pending: set[asyncio.Task[Any]]
    ...
    def get_hedging_stats(self) -> JSONDict:
        ...
        for r in successful:
            if len(r.latencies_ms) > 1:
                winner_latency = r.latencies_ms.get(r.winning_provider or "", 0)
                other_latencies = [
