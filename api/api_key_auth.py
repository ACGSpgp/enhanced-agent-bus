...
if TYPE_CHECKING:
    from .routes.signup import AccountRecord
...
_cached_get_account: Callable[[str], AccountRecord | None] | None = None
...
def _is_signup_key(api_key: str) -> bool:
    ...
    if _cached_get_account is None:
        try:
            from .routes.signup import get_account as imported_get_account

            _cached_get_account = imported_get_account
        except ImportError:
            return False
    get_account = _cached_get_account
    if get_account is None:
        return False
    return get_account(api_key) is not None
