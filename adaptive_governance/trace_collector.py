...
class TraceCollector:
    ...
    def collect_from_ledger_blocks(
        ...
        *inferred* from ``allowed``, ``violations``, and ``processing_time_ms``
        ...
        for block in blocks:
            ...
            if isinstance(block_data, dict) and block_data.get("type") != "genesis":
                audit_entries.append(block_data)
    ...
    @staticmethod
    def _entry_to_state(entry: dict[str, object]) -> int:
        ...
        if not entry.get("allowed", True):
            violations = cast(list[object], entry.get("violations", []))
            if len(violations) > 1:
                return IMPACT_TO_STATE[ImpactLevel.CRITICAL]
            return IMPACT_TO_STATE[ImpactLevel.HIGH]

        processing_time = float(cast(float | int | str, entry.get("processing_time_ms", 0.0)))
