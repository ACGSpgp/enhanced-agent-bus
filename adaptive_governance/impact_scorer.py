...
if TYPE_CHECKING:
    from enhanced_agent_bus._compat.types import MessagePayload, PolicyContext
else:
    try:
...
if available("sklearn"):
    _RandomForestRegressor = __import__(
...
mlflow: Any
if available("mlflow"):
    _mlflow = __import__(require("mlflow"))
    mlflow = _mlflow
...
sinkhorn_projection: Any
if available("governance_mhc"):
    _sinkhorn_projection = __import__(
        require("governance_mhc"), fromlist=["sinkhorn_projection"]
    ).sinkhorn_projection
    sinkhorn_projection = _sinkhorn_projection
else:
...
class ImpactScorer:
    ...
    def _apply_mhc_stability(self) -> None:
        """
    ...
    def _log_training_run_to_mlflow(
        self, X: Any, y: Any, recent_samples: list[tuple[ImpactFeatures, float]]
    ) -> None:
