...
from .context import TenantContext, tenant_context
...
class TenantManager:
    ...
    def __init__(
        ...
        # Event subscribers
        self._subscribers: dict[TenantEvent, list[Callable[..., Any]]] = {
            event: [] for event in TenantEvent
        }
    ...
    async def create_tenant(
        ...
        if auto_activate:
            activated_tenant = await self.activate_tenant(tenant.tenant_id)
            if activated_tenant is not None:
    ...
    async def get_tenant_hierarchy(self, tenant_id: str) -> list[Tenant]:
        ...
        while current_id:
            ...
            current_id = parent_id if isinstance(parent_id, str) else ""
