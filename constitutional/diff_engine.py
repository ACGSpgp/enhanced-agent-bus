...
class _CumulativeChanges(TypedDict):
    ...
    modified_fields: dict[str, JSONDict]
...
class ConstitutionalDiffEngine:
    ...
    def _stringify_principle(self, principle: object) -> str:
        ...
        if isinstance(principle, str):
            ...
            return json_dumps(principle, sort_keys=True)
        else:
    ...
    async def compute_multi_version_diff(self, version_ids: list[str]) -> _CumulativeDiff | None:
        ...
        for i in range(len(version_ids) - 1):
            ...
            # Accumulate changes
            cumulative_diff["cumulative_changes"]["added_fields"].update(diff.added_fields)
            cumulative_diff["cumulative_changes"]["removed_fields"].update(diff.removed_fields)
            cumulative_diff["cumulative_changes"]["modified_fields"].update(diff.modified_fields)
