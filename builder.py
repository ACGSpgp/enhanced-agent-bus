...
def build_sdpc_verifiers(config: BusConfiguration) -> SDPCVerifiers:
    ...
    try:
        ...
        from enhanced_agent_bus.deliberation_layer.intent_classifier import (
            IntentType as _IntentTypeClass,
        )
        ...
        class _NoOpEvolutionController:
            ...
            def reset_mutations(self, intent: object | None = None) -> None:
                _ = intent
        ...
        IntentType = cast(Any, _IntentType)
...
def build_pqc_service(config: BusConfiguration) -> PQCCryptoService | None:
    ...
    try:
        ...
        pqc_mode: Literal["classical_only", "hybrid", "pqc_only"] = (
            cast(Literal["classical_only", "hybrid", "pqc_only"], config.pqc_mode)
            if config.pqc_mode in ("classical_only", "hybrid", "pqc_only")
            else "classical_only"
        )

        verification_mode: Literal["strict", "classical_only", "pqc_only"] = (
            cast(Literal["strict", "classical_only", "pqc_only"], config.pqc_verification_mode)
        ...
        pqc_config = ConcretePQCConfig(
            ...
            kem_algorithm=cast(str, getattr(config, "pqc_key_algorithm", "kyber768")),
