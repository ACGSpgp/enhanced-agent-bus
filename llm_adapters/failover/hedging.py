...
HedgedTask: TypeAlias = asyncio.Task[tuple[str, object]]
...
class RequestHedgingManager:
    ...
    async def _wait_for_first_success(
        self, tasks: list[HedgedTask], errors: dict[str, str] | None = None
    ) -> tuple[str, object]:
        """Wait for the first successful task result."""
        done: set[HedgedTask] = set()
        pending: set[HedgedTask] = set(tasks)
        winner = None
        result = None
    ...
    def _process_completed_tasks(
        self, done: set[HedgedTask], pending: set[HedgedTask]
    ...
    def get_hedging_stats(self) -> JSONDict:
        ...
        for r in successful:
            if len(r.latencies_ms) > 1:
                winner_provider = r.winning_provider
                if winner_provider is None:
                    continue
                winner_latency = r.latencies_ms.get(winner_provider, 0)
                other_latencies = [
                    latency for p, latency in r.latencies_ms.items() if p != winner_provider
                ]
                if other_latencies:
