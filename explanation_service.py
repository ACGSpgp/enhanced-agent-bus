...
class ExplanationService:
    ...
    async def generate_explanation(
        ...
        if store_explanation and self.decision_store:
            await cast(Any, self.decision_store).store(explanation)
    ...
    def _calculate_single_factor(
        ...
        """
        try:
            method = cast(Callable[..., float], getattr(self.impact_scorer, scorer_method))
            return float(method(*args))
        except (AttributeError, KeyError, TypeError) as e:
            logger.debug(f"{factor_name} calculation failed, using default: {e}")
            return default
    ...
    def _calculate_scorer_factors(
        ...
        factor_configs: dict[str, tuple[str, tuple[object, ...], float]] = {
    ...
    def _calculate_overall_impact(
        ...
        """
        try:
            scorer = cast(Any, self.impact_scorer)
            return float(scorer.calculate_impact_score(message, context))
        except (AttributeError, KeyError, TypeError) as e:
            logger.debug(f"Impact score calculation failed, using fallback: {e}")
            additive_factors = [
                "semantic_score",
                "permission_score",
                "volume_score",
    ...
    def _get_governance_vector(
        ...
        if self.impact_scorer is not None:
            try:
                scorer_vector = cast(Any, self.impact_scorer).get_governance_vector(context)
                if scorer_vector:
                    governance_vector.update(scorer_vector)
                    return governance_vector
            except (AttributeError, KeyError, TypeError) as e:
                logger.debug(f"Governance vector retrieval failed, using defaults: {e}")
    ...
    def _extract_message_id(self, message: JSONDict | object) -> str | None:
        ...
        if isinstance(message, dict):
            ...
            return str(message_id) if isinstance(message_id, str) else None
    ...
    async def get_explanation(
        ...
        if self.decision_store:
            result = await cast(Any, self.decision_store).get(decision_id, tenant_id)
            return cast(DecisionExplanationV1 | None, result)
        return None
    ...
    async def explain(
        ...
        *,
        ...
        """Compatibility shim for older explanation port callers."""
        previous = self.enable_counterfactuals
        self.enable_counterfactuals = include_counterfactuals
        context: JSONDict = factors or {}
        verdict = str(context.get("verdict", "ALLOW"))
        try:
            ...
            self.enable_counterfactuals = previous
