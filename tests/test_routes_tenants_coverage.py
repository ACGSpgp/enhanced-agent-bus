...
class TestValidateJwtToken:
    def test_configured_jwt_algorithm_allows_hs256(self):
        with patch.dict(os.environ, {"JWT_ALGORITHM": "HS256"}, clear=False):
            assert _configured_jwt_algorithm() == "HS256"

    def test_configured_jwt_algorithm_invalid_raises(self):
        with patch.dict(os.environ, {"JWT_ALGORITHM": "NOPE"}, clear=False):
            with pytest.raises(ConfigurationError, match="JWT_ALGORITHM must be one of"):
                _configured_jwt_algorithm()
