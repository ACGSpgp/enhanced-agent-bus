...
try:
    ...
    class OPAClientProtocol(Protocol):  # type: ignore[no-redef]
        async def validate_constitutional(self, message: dict[str, object]) -> object: ...

    @runtime_checkable
    class RustProcessorProtocol(Protocol):  # type: ignore[no-redef]
        def validate(self, message: dict[str, object]) -> bool | dict[str, object]: ...
        async def validate_message(self, message: dict[str, object]) -> object: ...
        def constitutional_validate(self, provided_hash: str, expected_hash: str) -> bool: ...

    @runtime_checkable
    class PQCValidatorProtocol(Protocol):  # type: ignore[no-redef]
        def verify_governance_decision(
            self, decision: dict[str, object], signature: object, public_key: bytes
        ) -> bool: ...
...
try:
    ...
    PQCAlgorithm = None
    PQCSignature = None
    ConstitutionalHashValidator = None
...
class DynamicPolicyValidationStrategy:
    ...
    async def _validate_with_policy_client(self, message: AgentMessage) -> tuple[bool, str | None]:
        ...
        if policy_client is None:
            return False, "Policy client not available"
        result = await policy_client.validate_message_signature(message)
...
class OPAValidationStrategy:
    ...
    async def _validate_with_opa(self, message: AgentMessage) -> tuple[bool, str | None]:
        ...
        if opa_client is None:
            return False, "OPA client not available"
        result = await opa_client.validate_constitutional(message.to_dict())
        if not result.is_valid:
            return False, "; ".join(result.errors)
        return True, None
...
class RustValidationStrategy:
    ...
    async def _validate_with_rust_processor(self, message: AgentMessage) -> tuple[bool, str | None]:
        ...
        if rust_processor is None:
            return False, "Rust processor not available"
        # Attempt to use Rust processor's validation method
        # Check for validate_message method (preferred)
        if hasattr(rust_processor, "validate_message"):
            result = await rust_processor.validate_message(message.to_dict())
        ...
        if hasattr(rust_processor, "validate"):
            result = rust_processor.validate(message.to_dict())
            if isinstance(result, bool):
                if result:
                    return True, None
                return False, "Rust validation rejected message"
        ...
        if hasattr(rust_processor, "constitutional_validate"):
            result = rust_processor.constitutional_validate(
...
class PQCValidationStrategy:
    ...
    def __init__(
        ...
        self._constitutional_hash = CONSTITUTIONAL_HASH
        self._validator: PQCValidatorProtocol | None
...
class ConstitutionalValidationStrategy:
    ...
    async def _validate_with_constitutional_verifier(
        ...
        if isinstance(message.content, dict):
            context.update(
                {k: v for k, v in message.content.items() if isinstance(v, (int, bool, float))}
            )

        result = await verifier.verify_constitutional_compliance(
