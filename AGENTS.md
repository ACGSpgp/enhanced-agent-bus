...
- New JWT algorithm parsing in `enhanced_agent_bus` must go through
  `enhanced_agent_bus._compat.security.jwt_algorithms.resolve_jwt_algorithm`.
- Do not introduce new module-local JWT allowlists unless the module has an
  intentionally narrower posture than the shared helper, and that posture is
  documented next to the code and in operator-facing docs.
