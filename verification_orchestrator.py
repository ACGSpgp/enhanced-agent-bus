...
def _typed_timed(
    ...
    [Callable[_DecoratorParams, _DecoratorReturn]],
    Callable[_DecoratorParams, _DecoratorReturn],
]:
    return cast(
        Callable[
...
def _serialize_pqc_metadata(metadata: object | None) -> JSONDict | None:
    """Best-effort JSON serialization for optional PQC metadata objects."""
    if metadata is None:
        return None
    ...
    if callable(to_dict):
        ...
        if isinstance(serialized, dict):
            return cast(JSONDict, serialized)

    metadata_dict = getattr(metadata, "__dict__", None)
    if isinstance(metadata_dict, dict):
        return cast(JSONDict, dict(metadata_dict))
...
class VerificationOrchestrator:
    ...
    def __init__(
        ...
        self.asc_verifier: Any = None
        self.graph_check: Any = None
        self.pacar_verifier: Any = None
        self.evolution_controller: Any = None
        self.ampo_engine: Any = None
        self._IntentType: Any = None
        ...
        try:
            ...
            logger.warning(f"SDPC dependencies unavailable ({exc}); using no-op verifier stubs.")
            ...
            class _NoOpEvolutionController:
                ...
                def get_mutations(self, intent: object) -> list[Any]:
                    _ = intent
                    return []
        ...
        self._pqc_config: Any | None = None
        self._pqc_service: Any | None = None
        if self._enable_pqc:
    ...
    @_typed_timed("verification_orchestrate")
    async def verify(
        self,
        msg: AgentMessage,
        content_str: str,
    ...
    @_typed_timed("sdpc_verification")
    ...
    async def _perform_pqc(
        ...
        try:
                ...
                return (
                    ValidationResult(
                        ...
                        metadata={
                            ...
                            "pqc_metadata": _serialize_pqc_metadata(pqc_result.pqc_metadata),
            ...
            logger.error(f"PQC validation error: {exc}", exc_info=True)
            ...
            logger.warning(f"PQC validation failed, continuing with standard validation: {exc}")
    ...
    def _init_pqc(self, config: BusConfiguration) -> None:
        ...
        try:
            ...
            if config.pqc_mode in (
                ...
                pqc_mode = cast(Literal["classical_only", "hybrid", "pqc_only"], config.pqc_mode)
            ...
            if config.pqc_verification_mode in (
                ...
                verification_mode = cast(
                    Literal["strict", "classical_only", "pqc_only"],
            ...
            if phase_str.startswith("phase_"):
                migration_phase = cast(
                    Literal["phase_0", "phase_1", "phase_2", "phase_3", "phase_4", "phase_5"],
                    phase_str,
                )
            ...
            logger.warning(f"PQC libraries not available: {exc}. PQC validation disabled.")
            self._enable_pqc = False
        except PQC_OPERATION_ERRORS as exc:
            logger.error(f"Failed to initialize PQC service: {exc}. PQC validation disabled.")
            self._enable_pqc = False
