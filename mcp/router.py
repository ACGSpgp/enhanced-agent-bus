...
class MCPRouter:
    ...
    async def execute_tool(self, request: ToolRequest) -> ToolResponse:
        ...
        with _span(
            self._tracer,
            "mcp.router.execute_tool",
            cast(dict[str, str | int | float | bool], span_attrs),
        ):
            logger.info(
    ...
    def _resolve_server_id(self, request: ToolRequest) -> tuple[str | None, MCPTool | None]:
        ...
        # Index lookup
        resolved_tool: MCPTool | None = self._tool_index.get(request.tool_name)
        if resolved_tool is not None:
            return resolved_tool.server_id or "<pool>", resolved_tool
