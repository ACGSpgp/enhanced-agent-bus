...
MethodHandlerFunc = Callable[[JSONDict], Awaitable[JSONDict | None]]
...
class MACIEnforcerProtocol(Protocol):
    """Protocol for MACI enforcement used by the server."""
    ...
    async def validate_action(
        ...
        *,
        ...
        """Validate whether the agent may perform an action."""
        ...
...
class InternalTool:
    ...
    def __post_init__(self) -> None:
        """Normalize tool input schemas for stricter MCP validators."""
        self.input_schema = sanitize_tool_input_schema(self.input_schema)
...
class MCPIntegrationServer:
    ...
    def __init__(
        ...
        maci_enforcer: MACIEnforcerProtocol | None = None,
    ) -> None:
        """
        ...
        # Method handlers
        self._method_handlers: dict[str, MethodHandlerFunc] = {}
    ...
    async def handle_request(self, request: JSONDict) -> JSONDict | None:
        ...
        try:
            ...
            if self.validator and VALIDATORS_AVAILABLE:
                ...
                if operation_type:
                    agent_id_value = params.get("agent_id")
                    ...
                    context = MCPOperationContext(
                        ...
                        agent_id=agent_id_value if isinstance(agent_id_value, str) else "unknown",
                        tool_name=(
                            tool_name_value
                            if "tools" in method and isinstance(tool_name_value, str)
                            else None
                        ),
                        resource_uri=(
                            resource_uri_value
                            if "resources" in method and isinstance(resource_uri_value, str)
                            else None
                        ),
    ...
    async def _handle_tools_call(self, params: JSONDict) -> JSONDict:
        ...
        if self.config.enable_maci and self.maci_enforcer and MACI_AVAILABLE:
            try:
                agent_id_raw = arguments.get("_agent_id", "unknown")
                agent_id = agent_id_raw if isinstance(agent_id_raw, str) else "unknown"
                session_id_raw = arguments.get("_session_id")
                session_id = session_id_raw if isinstance(session_id_raw, str) else None
                synthesize_action = getattr(MACIAction, "SYNTHESIZE", "SYNTHESIZE")
    ...
    async def _handle_prompts_get(self, params: JSONDict) -> JSONDict:
        ...
        prompt_name = prompt_name_raw if isinstance(prompt_name_raw, str) else ""
    ...
    async def _handle_logging_set_level(self, params: JSONDict) -> JSONDict:
        ...
        level = level_raw.upper() if isinstance(level_raw, str) else "INFO"
