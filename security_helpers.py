...
def validate_tenant_consistency(
    agents_registry: dict[str, dict[str, object]],
    from_agent: str,
    to_agent: str | None,
    message_tenant: str | None,
) -> list[str]:
    ...
    if from_agent in agents_registry:
        sender_value = agents_registry[from_agent].get("tenant_id")
        sender_tenant = normalize_tenant_id(sender_value if isinstance(sender_value, str) else None)
    ...
    if to_agent and to_agent in agents_registry:
        recipient_value = agents_registry[to_agent].get("tenant_id")
        recipient_tenant = normalize_tenant_id(
            recipient_value if isinstance(recipient_value, str) else None
        )
