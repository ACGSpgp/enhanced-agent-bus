...
class ImpactScorer(Protocol):
    """Runtime protocol for scorers used by the service."""

    def score(self, context: JSONDict) -> ScoringResult:
        """Compute a score for the provided context."""
        ...


class UnloadableImpactScorer(ImpactScorer, Protocol):
    """Scorer that can release heavyweight resources."""
...
class ImpactScoringService:
    ...
    def __init__(self, config: ImpactScoringConfig | None = None):
        ...
        self._minicpm_scorer: UnloadableImpactScorer | None = None
        self._minicpm_available = False

        # Initialize base scorers
        self.scorers: dict[ScoringMethod, ImpactScorer] = {
    ...
    def get_minicpm_score(self, context: JSONDict) -> ScoringResult | None:
        ...
        if not self._minicpm_available or self._minicpm_scorer is None:
            return None
        return self._minicpm_scorer.score(context)
