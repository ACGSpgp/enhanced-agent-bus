...
_configured_collaboration_jwt_algorithm = _api_integration._configured_collaboration_jwt_algorithm
...
class TestValidateToken:
    ...
    def test_valid_token_returns_user_data(self):
        ...
        with patch("importlib.import_module", return_value=mock_jwt):
            with patch.dict(os.environ, {"COLLABORATION_JWT_ALGORITHM": "HS256"}, clear=False):
    ...
    def test_configured_collaboration_jwt_algorithm_defaults_to_hs256(self):
        with patch.dict(os.environ, {}, clear=True):
            assert _configured_collaboration_jwt_algorithm() == "HS256"

    def test_configured_collaboration_jwt_algorithm_rejects_rs256(self):
        with patch.dict(os.environ, {"COLLABORATION_JWT_ALGORITHM": "RS256"}, clear=False):
            with pytest.raises(
                Exception,
                match="COLLABORATION_JWT_ALGORITHM must be 'HS256'",
            ):
                _configured_collaboration_jwt_algorithm()
...
class TestCollaborationAlgorithmPolicy:
    def test_collaboration_policy_is_hs256_only(self):
        with patch.dict(os.environ, {"COLLABORATION_JWT_ALGORITHM": "HS256"}, clear=False):
            assert _configured_collaboration_jwt_algorithm() == "HS256"
